...
test('el buscador territorial reutiliza el directorio público y devuelve un conjunto navegable', async () => {
  ...
  assert.match(source, /brotherhoods_by_day/)
  assert.match(source, /localityLabel\(item\)/)
  assert.match(source, /Ver la Semana Santa por jornadas/)
  ...
  assert.match(source, /\/hermandades\/sacramentales\/\$\{localitySlug\(sacramental\)\}/)
