...
test('la navegación contextual mantiene activa y accesible la sección visible', () => {
  ...
  assert.match(component, /nav\.querySelector\('\[aria-current="page"\]'\)/)
  assert.match(component, /active\.getBoundingClientRect\(\)/)
  assert.match(component, /nav\.getBoundingClientRect\(\)/)
  assert.match(component, /nav\.scrollTo/)
  assert.match(component, /aria-current=\{active \? 'page' : undefined\}/)
  assert.doesNotMatch(component, /item\.mark/)
  assert.match(styles, /font-size: 13px/)
  assert.match(styles, /min-height: 40px/)
  assert.match(styles, /\.scrollButton/)
  assert.match(styles, /\.mobilePicker/)
})
