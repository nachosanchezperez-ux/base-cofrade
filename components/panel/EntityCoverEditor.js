...
export default function EntityCoverEditor({
  entity,
  cover,
  canEdit,
  action,
  ...
  const [dimensions, setDimensions] = useState({
    width: asset.width_px || 0,
    height: asset.height_px || 0,
  })
  ...
  const resolvedFit = useMemo(() => {
    ...
    return entity.type === 'step' && dimensions.height > dimensions.width * 1.12
      ? 'contain'
      : 'cover'
    ...
    startTransition(async () => {
      ...
      try {
        if (hasFile) {
          ...
          const { error } = await supabase.storage.from('hilo-media').upload(uploadedPath, file, {
  ...
  return (
    <form onSubmit={handleSubmit} className={`${styles.panelCard} ${styles.editorForm} ${styles.coverEditor}`}>
      ...
      <div className={styles.coverWorkspace}>
        <div className={styles.coverPreviews}>
          <div>
            <span>Escritorio · encuadre editorial</span>
            <div className={`${styles.coverPreview} ${styles[`coverPreview_${entity.type}`]}`}>
          ...
          <div>
            ...
            <div className={`${styles.coverPreview} ${styles.coverPreviewMobile}`}>
              {previewUrl ? <img src={previewUrl} alt="Previsualización móvil" style={mobileStyle} /> : <b>Sin portada</b>}
            </div>
          </div>
        </div>

        <div className={`${styles.editorForm} ${styles.coverControls}`}>
          ...
          <div className={styles.coverRangeGrid}>
      ...
      <div className={`${styles.formGrid} ${styles.coverMetadata}`}>
        ...
        <label><span>Estado de derechos</span><select name="rights_status" defaultValue={asset.rights_status || 'authorized'} required disabled={!canEdit}><option value="owned">Propia</option><option value="authorized">Autorizada</option><option value="licensed">Con licencia</option><option value="public_domain">Dominio público</option></select></label>
