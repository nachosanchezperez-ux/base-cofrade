  ...
  return (
    ...
    >
      <Image
        ...
        alt={visual.alt || ''}
        fill
        sizes={photo ? '(max-width: 859px) 96px, (max-width: 1039px) 112px, 168px' : '(max-width: 859px) 78px, 96px'}
        ...
        <div className={`${styles.grid} ${polishStyles.todayGrid}`}>
          {cards.map((card, index) => (
            <article
              ...
              data-home-today-role={index === 0 ? 'lead' : 'secondary'}
            >
          ...
          {content?.march ? (
            <article className={`${styles.musicCard} ${polishStyles.todayMusic}`} data-home-today-role="music">
