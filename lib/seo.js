...
export const INDEXABLE_DIRECTORY_MIN_ITEMS = 3;
...
export function directoryRobots(items = []) {
  return {
    index: items.length >= INDEXABLE_DIRECTORY_MIN_ITEMS,
    follow: true,
  };
}
...
export function bandSeoTitle(band) {
  const formation = String(band.type || '').trim();
  const name = String(band.popularName || band.officialName || 'Banda cofrade').trim();
  const base = formation && !name.toLocaleLowerCase('es').startsWith(formation.toLocaleLowerCase('es'))
    ? `${formation} ${name}`
    : name;
  const municipality = String(band.municipality || '').trim();
  const includesMunicipality = municipality
    && base.toLocaleLowerCase('es').includes(municipality.toLocaleLowerCase('es'));

  return `${base}${municipality && !includesMunicipality ? ` (${municipality})` : ''}`;
}
