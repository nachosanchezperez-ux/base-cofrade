...
function photoFiles(formData) {
  const photos = formData
    .getAll('photos')
    .filter((file) => file && typeof file.arrayBuffer === 'function' && file.size > 0)

  if (photos.length > CONTRIBUTION_LIMITS.photos) {
    throw new ContributionValidationError(`Puedes adjuntar hasta ${CONTRIBUTION_LIMITS.photos} fotografías.`)
  }

  let totalBytes = 0
  for (const photo of photos) {
    if (!CONTRIBUTION_PHOTO_TYPES.has(photo.type)) {
      throw new ContributionValidationError('Las fotografías deben ser JPG, PNG o WebP.')
    }
    if (photo.size > CONTRIBUTION_LIMITS.photoBytes) {
      throw new ContributionValidationError('Cada fotografía puede ocupar como máximo 5 MB.')
    }
    totalBytes += photo.size
  }
  if (totalBytes > CONTRIBUTION_LIMITS.totalPhotoBytes) {
    throw new ContributionValidationError('Las fotografías pueden ocupar como máximo 10 MB en total.')
  }
  return photos
}
...
export function parseContributionForm(formData) {
  ...
  if (contributionType === 'media' && !photos.length && !sources.length) {
    throw new ContributionValidationError('Adjunta una fotografía o enlaza el documento que quieres aportar.')
  }
  if (photos.length && !photoCredit) {
    throw new ContributionValidationError('Indica la autoría o el crédito de las fotografías.')
  }
  if (photos.length && !formData.has('rights_confirmed')) {
  ...
  return {
    ...
    rightsConfirmed: photos.length ? true : formData.has('rights_confirmed'),
    photos,
  }
}
