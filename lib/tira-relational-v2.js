...
export function relationalV2Intent(question = '', context = null) {
  ...
  if (setType === 'brotherhood') {
    ...
    if (asksWhich && asksMusic && possession && !negative) return { kind: 'brotherhoods_with_music' }
  }
  ...
  const asksMusicRelation = /\b(banda|bandas|musica|acompanamiento|acompanamientos|acompan[a-z]*)\b/.test(q)
  const historical = /\b(historic|historica|historicas|historico|historicos|historia|anteriores|anteriormente|antes|acompanaron|acompanaba|acompanaban|ha acompanado|han acompanado|a lo largo)\b/.test(q)
  if (asksMusicRelation && historical) return { kind: 'music_history' }
  if (context?.entityType === 'band' && /^(?:y )?(?:antes|historicamente|anteriormente)\b/.test(q)) {
    return { kind: 'music_history' }
  }

  const asksBrotherhoodTargets = /\b(a que hermandades|en que hermandades|que hermandades|cuales hermandades|donde)\b/.test(q)
  const currentBandVerb = /\b(acompana|acompanan|toca|tocan|suena|suenan)\b/.test(q)
  if ((asksBrotherhoodTargets && currentBandVerb) || (context?.entityType === 'band' && /\b(hermandades|donde)\b/.test(q) && currentBandVerb)) {
    return { kind: 'current_band_brotherhoods' }
  }
...
export function relationalPeriodLabel(row = {}) {
  ...
  if (start && row.is_current && /vigent/i.test(start)) return start
