...
function coversSeason(item, seasonYear) {
  return Boolean(item.yearTo)
    && item.yearTo >= seasonYear
...
export function partitionAccompanimentsBySeason(currentItems = [], historicalItems = [], currentYear) {
  ...
  const upcomingItems = hasValidSeason
    ...
    : []
  const seasonHistoricalItems = hasValidSeason
    ? normalizedHistoricalItems.filter((item) => coversSeason(item, seasonYear))
    : []
  const outsideSeasonHistoricalItems = hasValidSeason
    ? normalizedHistoricalItems.filter((item) => !coversSeason(item, seasonYear))
  ...
  return {
    current: hasValidSeason
      ? [
          ...
          ...seasonHistoricalItems,
        ]
      : [...normalizedCurrentItems],
    upcoming: upcomingItems,
    historical: [...outsideSeasonHistoricalItems, ...endedItems],
  }
}
