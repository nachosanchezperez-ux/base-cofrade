...
function firstHolyWeekMoment(value = '') {
...
export function presentAccompanimentMoment(item = {}) {
  const outingType = formatOutingType(item.outingType)
  const documentedMoment = firstHolyWeekMoment(outingType)
  if (documentedMoment) return documentedMoment

  if (GENERIC_HOLY_WEEK_KEYS.has(outingTypeKey(outingType))) {
    return firstHolyWeekMoment(item.notes) || 'Semana Santa'
  }
