export const HOLY_WEEK_DAYS = [
  'Viernes de Dolores',
  'Sábado de Pasión',
  'Domingo de Ramos',
  'Lunes Santo',
  'Martes Santo',
  'Miércoles Santo',
  'Jueves Santo',
  'Madrugada',
  'Viernes Santo',
  'Sábado Santo',
  'Domingo de Resurrección',
]

export const MONTHS = [
  'Enero',
  'Febrero',
  'Marzo',
  'Abril',
  'Mayo',
  'Junio',
  'Julio',
  'Agosto',
  'Septiembre',
  'Octubre',
  'Noviembre',
  'Diciembre',
]

export const DIRECTORY_TYPES = [
  {
    key: 'semana-santa',
    type: 'Penitencia',
    label: 'Semana Santa',
    description: 'Hermandades organizadas por localidad y jornada de salida.',
    href: '/hermandades/semana-santa',
    icon: '/iconos/hermandades/penitencia.png',
  },
  {
    key: 'gloria',
    type: 'Gloria',
    label: 'Glorias',
    description: 'Devociones organizadas por localidad y mes principal.',
    href: '/hermandades/gloria',
    icon: '/iconos/hermandades/gloria.png',
  },
  {
    key: 'sacramentales',
    type: 'Sacramental',
    label: 'Sacramentales',
    description: 'Corporaciones sacramentales reunidas por localidad.',
    href: '/hermandades/sacramentales',
    icon: '/iconos/hermandades/sacramental.png',
  },
  {
    key: 'agrupaciones-parroquiales',
    type: 'Agrupación Parroquial',
    label: 'Agrupaciones Parroquiales',
    description: 'Corporaciones parroquiales organizadas por localidad.',
    href: '/hermandades/agrupaciones-parroquiales',
    icon: '/iconos/hermandades/agrupacion-parroquial.svg',
    itemSingular: 'agrupación',
    itemPlural: 'agrupaciones',
  },
]

export function normalizeDirectoryValue(value = '') {
  return String(value)
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
    .trim()
}

export function directorySlug(value = '') {
  return normalizeDirectoryValue(value)
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
}

export function directoryType(key) {
  return DIRECTORY_TYPES.find((item) => item.key === key) || null
}

export function hasDirectoryType(hermandad, key) {
  const config = directoryType(key)
  if (!config) return false

  if (key === 'semana-santa' && holyWeekDay(hermandad)) return true

  return (hermandad.tipos || []).some(
    (tipo) => normalizeDirectoryValue(tipo) === normalizeDirectoryValue(config.type)
  )
}

export function localitySlug(hermandad) {
  if (normalizeDirectoryValue(hermandad.localidad) === 'sevilla') return 'sevilla-capital'
  return directorySlug(hermandad.localidad)
}

export function localityLabel(hermandad) {
  if (normalizeDirectoryValue(hermandad.localidad) === 'sevilla') return 'Sevilla capital'
  return hermandad.localidad || ''
}

export function holyWeekDay(hermandad) {
  const processionDay = normalizeDirectoryValue(hermandad.diaSalida)
  if (!processionDay) return ''

  if (processionDay.includes('madrugada')) return 'Madrugada'

  const explicitDay = HOLY_WEEK_DAYS.find((day) => (
    day !== 'Madrugada'
    && processionDay.includes(normalizeDirectoryValue(day))
  ))

  if (explicitDay) return explicitDay
  if (processionDay.includes('madruga')) return 'Madrugada'

  return ''
}

export function gloryMonth(hermandad) {
  const structuredMonth = Number(hermandad.gloriaMes)
  if (Number.isInteger(structuredMonth) && structuredMonth >= 1 && structuredMonth <= 12) {
    return MONTHS[structuredMonth - 1]
  }

  const processionDay = normalizeDirectoryValue(hermandad.gloriaFecha || hermandad.diaSalida)
  return MONTHS.find((month) => processionDay.includes(normalizeDirectoryValue(month))) || ''
}

export function directoryPeriod(hermandad, key) {
  if (key === 'semana-santa') return holyWeekDay(hermandad)
  if (key === 'gloria') return gloryMonth(hermandad)
  return ''
}

export function directoryPath(hermandad, key) {
  const locality = localitySlug(hermandad)
  if (!locality) return ''

  if (key === 'sacramentales' || key === 'agrupaciones-parroquiales') {
    return `/hermandades/${key}/${locality}`
  }

  const period = directorySlug(directoryPeriod(hermandad, key))
  if (!period) return ''
  return `/hermandades/${key}/${locality}/${period}`
}

export function directoryContextLabel(hermandad, key) {
  const period = directoryPeriod(hermandad, key)
  if (period) return period
  if (key === 'sacramentales') return 'Sacramental'
  if (key === 'agrupaciones-parroquiales') return 'Agrupación Parroquial'
  return ''
}

export function displayName(hermandad) {
  if (hermandad.slug === 'asuncion-de-cantillana') return 'La Asunción de Cantillana'
  return hermandad.nombrePopular
}

export function labelFromSlug(slug = '') {
  const knownLabels = [...HOLY_WEEK_DAYS, ...MONTHS]
  const known = knownLabels.find((label) => directorySlug(label) === slug)
  if (known) return known
  if (slug === 'sevilla-capital') return 'Sevilla capital'

  return slug
    .split('-')
    .filter(Boolean)
    .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
    .join(' ')
}

function compareText(first = '', second = '') {
  return String(first).localeCompare(String(second), 'es', { sensitivity: 'base' })
}

function indexedPeriodRank(value, periods) {
  const normalized = normalizeDirectoryValue(value)
  const index = periods.findIndex((period) => normalizeDirectoryValue(period) === normalized)
  return index === -1 ? periods.length : index
}

function territoryRank(hermandad) {
  return normalizeDirectoryValue(hermandad.localidad) === 'sevilla' ? 0 : 1
}

export function sortBrotherhoods(items = [], typeKey = '') {
  return [...items].sort((first, second) => {
    const territoryDifference = territoryRank(first) - territoryRank(second)
    if (territoryDifference) return territoryDifference

    const localityDifference = compareText(localityLabel(first), localityLabel(second))
    if (localityDifference) return localityDifference

    if (typeKey === 'semana-santa') {
      const firstRank = indexedPeriodRank(directoryPeriod(first, typeKey), HOLY_WEEK_DAYS)
      const secondRank = indexedPeriodRank(directoryPeriod(second, typeKey), HOLY_WEEK_DAYS)
      if (firstRank !== secondRank) return firstRank - secondRank
    }

    if (typeKey === 'gloria') {
      const firstRank = indexedPeriodRank(directoryPeriod(first, typeKey), MONTHS)
      const secondRank = indexedPeriodRank(directoryPeriod(second, typeKey), MONTHS)
      if (firstRank !== secondRank) return firstRank - secondRank
    }

    return compareText(displayName(first), displayName(second))
  })
}
