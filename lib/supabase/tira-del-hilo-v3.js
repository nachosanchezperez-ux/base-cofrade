...
function imageSetFollowUpIntent(question, context) {
  ...
  if (/\b(quien|autoria|autor|autores|hizo|realizo|creo)\b/.test(q) && refersToSet) {
...
async function authorshipOfImageSet(supabase, resolved, sourceIntent) {
  ...
  return answer({
    ...
    context: resultSetContext({ root, entities, sourceIntent }),
  })
}
...
async function restorationsOfImageSet(supabase, resolved, sourceIntent) {
  ...
  const result = await supabase
    ...
    .select('image_entity_id, title, update_date, year, discipline')
    .in('image_entity_id', entities.map((entity) => entity.id))
  if (result.error) throw result.error
  ...
  return answer({
    ...
    items: restored.map((entity) => {
      const details = (rowsByImage.get(entity.id) || [])
        .map((row) => [row.year || row.update_date || '', row.discipline, row.title].filter(Boolean).join(' · '))
        .join(' / ')
...
async function compareImageSet(supabase, resolved, sourceIntent) {
  ...
  const [imageRowsResult, authorshipResult, restorationResult] = await Promise.all([
    supabase
      .from('images')
      .select('entity_id, image_type, execution_date, execution_date_text, material, technique')
      .in('entity_id', ids),
    supabase
      .from('image_authorship_details')
  ...
  return answer({
    text: `Comparo las ${entities.length} ${entities.length === 1 ? 'imagen' : 'imágenes'} que veníamos consultando con los campos documentados de forma homogénea: datación, tipología, técnica o material, autoría y restauraciones publicadas.`,
    ...
    items: entities.map((entity) => {
      ...
      return {
        ...
        meta: [
          row.image_type,
          row.execution_date_text || row.execution_date || 'Datación no documentada',
...
export async function askHiloCofradeV3(question, context = null) {
  ...
  if (setIntent) {
    ...
    try {
      ...
      if (setIntent === 'image_set_authorship') return authorshipOfImageSet(supabase, resolved, sourceIntent)
      if (setIntent === 'image_set_restorations') return restorationsOfImageSet(supabase, resolved, sourceIntent)
      if (setIntent === 'image_set_compare') return compareImageSet(supabase, resolved, sourceIntent)
    } catch (error) {
