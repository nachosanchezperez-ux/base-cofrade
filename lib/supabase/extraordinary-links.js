...
async function publishedEntityIdBySlug(supabase, slug, entityType) {
  if (!slug) return ''
  const { data, error } = await supabase
    .from('entities')
    .select('id')
...
export async function getBrotherhoodUpcomingExtraordinaryLinksBySlug(slug) {
  try {
    const supabase = createPublicClient()
    const brotherhoodId = await publishedEntityIdBySlug(supabase, slug, 'brotherhood')
...
export async function getBandUpcomingExtraordinaryLinksBySlug(slug) {
  try {
    const supabase = createPublicClient()
    const bandId = await publishedEntityIdBySlug(supabase, slug, 'band')
    if (!bandId) return []
    const { data, error } = await supabase
