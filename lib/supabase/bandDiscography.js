...
export async function getBandDiscography(
  bandEntityId,
  { bandName = '', bandLogoPath = '', artistSpotifyUrl = '' } = {}
) {
  ...
  try {
    ...
    const resolvedArtistSpotifyUrl = artistSpotifyUrl || spotifyArtistResult.data?.url || ''
    const resolvedBandName = bandName || bandNameResult.data?.name || bandEntityResult.data?.name || ''
    const resolvedBandLogoPath = bandLogoPath || bandProfileResult.data?.logo_path || ''
    ...
    return releases.map((release) => ({
      ...
      artistSpotifyUrl: resolvedArtistSpotifyUrl,
      bandName: resolvedBandName,
      bandLogoPath: resolvedBandLogoPath,
      tracks: tracks
