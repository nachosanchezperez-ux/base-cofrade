...
async function loadBandDiscography(bandEntityId) {
...
export const getBandDiscography = cache((bandEntityId) => {
  if (!bandEntityId) return []

  return getCachedPublicData({
    key: ['band-discography', bandEntityId],
    tags: [
      PUBLIC_CACHE_TAGS.BANDS,
      PUBLIC_CACHE_TAGS.AGENTS,
      publicEntityTag('band-id', bandEntityId),
    ],
    loader: () => loadBandDiscography(bandEntityId),
  })
})
