...
function routeRef(href = '') {
  const match = String(href).match(/^\/(hermandades|imagenes|pasos|bandas)\/([^/?#]+)/)
  ...
  try {
    ...
    const [brotherhoodRows, bandRows] = await Promise.all([
      ...
      missingBandIds.length
        ? assertRows(
            await supabase.from('bands').select('entity_id, logo_path, hero_image_path, hero_image_alt').in('entity_id', missingBandIds),
            'No se pudieron consultar los recursos visuales de las bandas'
          )
        : [],
    ])
    ...
    for (const row of bandRows) {
      const path = row.hero_image_path || row.logo_path || ''
      if (!path || visualByEntity.has(row.entity_id)) continue
      const entity = entityById.get(row.entity_id)
      if (!entity) continue
      const isPhoto = Boolean(row.hero_image_path)
      visualByEntity.set(entity.id, {
        entityId: entity.id,
        entityType: entity.entity_type,
        name: entity.name,
        href: entityHref(entity),
        path: publicPath(supabase, path),
        alt: isPhoto ? row.hero_image_alt || entity.name : `Logotipo de ${entity.name}`,
        credit: '',
        kind: isPhoto ? 'photo' : 'logo',
        fitMode: isPhoto ? 'cover' : 'contain',
        focusPosition: '50% 50%',
      })
    }
...
export async function getHomeVisualMediaForHrefs(hrefs = []) {
  const refs = [...new Set((hrefs || []).map(routeRef).filter(Boolean).map((item) => item.key))]
  if (!refs.length) return new Map()

  try {
    const supabase = createClient()
    const parsed = refs.map((key) => {
      const separator = key.indexOf(':')
      return { entityType: key.slice(0, separator), slug: key.slice(separator + 1), key }
    })
    ...
    const entities = assertRows(
      await supabase
        ...
        .eq('status', 'published'),
      'No se pudieron resolver las rutas visuales de la Home'
    )

    const visualById = await getHomeVisualMediaMap(entities.map((entity) => entity.id))
    const entityByKey = new Map(entities.map((entity) => [`${entity.entity_type}:${entity.slug}`, entity]))
    const visualByRoute = new Map()

    for (const item of parsed) {
      const entity = entityByKey.get(item.key)
      const visual = entity ? visualById.get(entity.id) : null
      if (visual) visualByRoute.set(item.key, visual)
    }

    return visualByRoute
  } catch (error) {
    console.error('[Hilo Cofrade] No se pudieron resolver los visuales desde rutas públicas', {
      error: error instanceof Error ? error.message : String(error),
    })
    return new Map()
  }
...
export function homeVisualRouteKey(href = '') {
  return routeRef(href)?.key || ''
}
