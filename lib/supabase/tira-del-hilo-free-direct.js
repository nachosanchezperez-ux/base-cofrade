...
import { freeDirectIntent, normalizeFreeDirectText } from '@/lib/tira-free-direct'
...
async function marchDedication(supabase, march) {
  ...
  const items = rows.map((row) => {
    ...
    return {
      label,
      meta: [row.dedication_type, row.date_from_text, entity && row.dedication_text !== entity.name ? row.dedication_text : ''].filter(Boolean).join(' · '),
      href: entityHref(entity),
    }
  })
  ...
  return answer({
    ...
    evidence: rows.slice(0, 4).map((row, index) => ({
      ...
      detail: [items[index]?.label, row.dedication_type, row.date_from_text].filter(Boolean).join(' · '),
    })),
    references,
...
async function marchPremiere(supabase, march) {
  ...
  if (!dateLabel && !row.premiere_place_id && !row.premiered_by_band_entity_id) return null
