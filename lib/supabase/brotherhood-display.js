...
async function filterBrotherhoodSourceScope(supabase, hermandad) {
  ...
  const [interventionsResult, processionStatsResult] = await Promise.all([
  ...
  const scopedSourceIds = new Set(
    uuidIds([
      ...sourceLinkResults.flatMap((result) => {
        if (result.error) {
  ...
  return sources.filter((source) => (
    !UUID_PATTERN.test(String(source.id || '')) || scopedSourceIds.has(source.id)
  ))
}
