...
async function filterBrotherhoodSourceScope(supabase, hermandad) {
  const sources = hermandad.fuentesFicha || []
  const databaseSourceIds = uuidIds(sources.map((source) => source.id))
  if (!databaseSourceIds.length) return sources

  const imageIds = uuidIds((hermandad.imagenes || []).map((item) => item.id))
  const stepIds = uuidIds((hermandad.pasos || []).map((item) => item.id))
  const heritageIds = uuidIds((hermandad.patrimonio || []).map((item) => item.id))
  const eventIds = uuidIds((hermandad.cronologia || []).map((item) => item.id))
  const entityTargetIds = uuidIds([hermandad.id, ...imageIds, ...stepIds, ...heritageIds, ...eventIds])
  const heritageUpdateIds = uuidIds((hermandad.estrenos || []).map((item) => item.id))
  const stepPhaseIds = uuidIds(
    (hermandad.pasos || []).flatMap((step) => (step.fases || []).map((phase) => phase.id))
  )
  const cultIds = uuidIds((hermandad.cultos || []).map((item) => item.id))
  const outingIds = uuidIds((hermandad.salidas || []).map((item) => item.id))
  const musicPeriodIds = uuidIds((hermandad.acompanamientoActual || []).map((item) => item.id))
  const visibleAgentIds = uuidIds([
    ...(hermandad.pasos || []).flatMap((step) => (
      (step.fases || []).flatMap((phase) => (phase.agentes || []).map((agent) => agent.id))
    )),
    ...(hermandad.patrimonio || []).flatMap((item) => (item.agentes || []).map((agent) => agent.id)),
    ...(hermandad.estrenos || []).flatMap((item) => (item.agentes || []).map((agent) => agent.id)),
  ])

  const [authorRelationsResult, interventionsResult, processionStatsResult] = await Promise.all([
    imageIds.length
      ? supabase
    ...
    entityTargetIds.length
      ? supabase
          .from('heritage_interventions')
          .select('id')
          .in('target_entity_id', entityTargetIds)
          .eq('status', PUBLISHED_STATUS)
      : Promise.resolve({ data: [], error: null }),
    supabase
  ...
  if (authorRelationsResult.error) {
    throw new Error(`No se pudo acotar las fuentes de autoría: ${authorRelationsResult.error.message}`)
  }
  if (interventionsResult.error) {
    throw new Error(`No se pudo acotar las fuentes de intervenciones: ${interventionsResult.error.message}`)
  }
  if (processionStatsResult.error) {
    throw new Error(`No se pudo acotar la fuente de datos de jornada: ${processionStatsResult.error.message}`)
  }

  const agentIds = uuidIds([
    ...visibleAgentIds,
    ...(authorRelationsResult.data || []).map((item) => item.source_entity_id),
  ])
  const interventionIds = uuidIds((interventionsResult.data || []).map((item) => item.id))
  const sourceLinkResults = await Promise.all([
    entityTargetIds.length
      ? supabase.from('source_links').select('source_id').in('entity_id', entityTargetIds)
      : Promise.resolve({ data: [], error: null }),
    ...
    interventionIds.length
      ? supabase.from('source_links').select('source_id').in('intervention_id', interventionIds)
      : Promise.resolve({ data: [], error: null }),
    stepPhaseIds.length
      ? supabase.from('source_links').select('source_id').in('step_phase_id', stepPhaseIds)
      : Promise.resolve({ data: [], error: null }),
    cultIds.length
      ? supabase.from('source_links').select('source_id').in('cult_id', cultIds)
      : Promise.resolve({ data: [], error: null }),
    outingIds.length
      ? supabase.from('source_links').select('source_id').in('outing_id', outingIds)
      : Promise.resolve({ data: [], error: null }),
    outingIds.length
      ? supabase.from('source_links').select('source_id').in('outing_series_id', outingIds)
      : Promise.resolve({ data: [], error: null }),
    ...
    agentIds.length
      ? supabase
          ...
          .in('source_id', databaseSourceIds)
      : Promise.resolve({ data: [], error: null }),
  ])

  const scopedLinkResults = sourceLinkResults.slice(0, -1)
  const agentSourceResult = sourceLinkResults[sourceLinkResults.length - 1]
  const scopedSourceIds = new Set(
    uuidIds([
      ...scopedLinkResults.flatMap((result) => {
        if (result.error) {
          throw new Error(`No se pudieron acotar los enlaces de fuentes: ${result.error.message}`)
        }
        return (result.data || []).map((item) => item.source_id)
      }),
      processionStatsResult.data?.source_id,
    ])
  )

  if (agentSourceResult.error) {
    throw new Error(`No se pudieron identificar las fuentes globales de agentes: ${agentSourceResult.error.message}`)
  }
  const agentSourceIds = new Set(
    uuidIds((agentSourceResult.data || []).map((item) => item.source_id))
  )

  return sources.filter((source) => (
    !UUID_PATTERN.test(String(source.id || ''))
    || !agentSourceIds.has(source.id)
...
export async function getHermandadPageBySlug(slug) {
  ...
  const [fuentesFicha, display] = await Promise.all([
    filterBrotherhoodSourceScope(supabase, baseHermandad).catch((error) => {
      console.error('[Hilo Cofrade] No se pudo acotar el ámbito de fuentes de la hermandad', {
        slug,
        error: error instanceof Error ? error.message : String(error),
      })
      return baseHermandad.fuentesFicha || []
    }),
    enrichBrotherhoodDisplay(hermandad).catch((error) => {
