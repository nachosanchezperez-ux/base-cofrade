...
import { classifyBrotherhoodSourceLinks } from '@/lib/brotherhood-source-scope'
...
async function filterBrotherhoodSourceScope(supabase, hermandad) {
  ...
  const sourceLinksResult = await supabase
    .from('source_links')
    .select('source_id, entity_id, heritage_update_id, intervention_id, step_phase_id, cult_id, outing_id, outing_series_id, music_accompaniment_period_id')
    .in('source_id', databaseSourceIds)

  if (sourceLinksResult.error) {
    throw new Error(`No se pudieron acotar los enlaces de fuentes: ${sourceLinksResult.error.message}`)
  }

  const {
    scopedSourceIds,
    agentSourceIds,
  } = classifyBrotherhoodSourceLinks(sourceLinksResult.data || [], {
    entityTargetIds,
    heritageUpdateIds,
    interventionIds,
    stepPhaseIds,
    cultIds,
    outingIds,
    // Mantiene el contrato histórico: este segundo pase comprobaba outing_series_id
    // contra los IDs de salidas ya presentes en la ficha.
    outingSeriesTargetIds: outingIds,
    musicPeriodIds,
    agentIds,
  })

  if (processionStatsResult.data?.source_id) {
    scopedSourceIds.add(processionStatsResult.data.source_id)
  }
