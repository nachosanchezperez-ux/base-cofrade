...
const EXPLICIT_ENTITY_THRESHOLD = 300
...
async function bestEntity(supabase, question, types, context = null) {
  ...
  const explicit = ranked[0] || null

  // Un nombre escrito en la pregunta siempre prevalece sobre el contexto previo.
  // Esto evita que una repregunta sobre otra persona o entidad herede por error
  // el sujeto de la respuesta anterior.
  if (explicit?.score >= EXPLICIT_ENTITY_THRESHOLD) return explicit.entity

  if (context?.entityId && (!types?.length || types.includes(context.entityType))) {
    const normalizedQuestion = normalize(question)
    const looksLikeFollowUp = /^(y|ademas|tambien|entonces|que|quien|cual|cuales|cuanto|cuantos|cuantas|donde|como)\b/.test(normalizedQuestion)
...
async function brotherhoodsOfBand(supabase, question, context) {
  ...
  const stepById = new Map(steps.map((item) => [item.id, item]))
  const brotherhoodCount = new Set(rows.map((item) => item.brotherhood_entity_id)).size

  return answer({
    text: `${band.name} aparece actualmente relacionada con ${brotherhoodCount} ${brotherhoodCount === 1 ? 'hermandad' : 'hermandades'} mediante acompañamientos publicados.`,
...
async function titularsOfBrotherhood(supabase, question, context) {
  ...
  const result = await supabase
    ...
    .select('image_entity_id, relation_type')
    .eq('brotherhood_entity_id', brotherhood.id)
...
async function genericEntity(supabase, question, context) {
  ...
  if (!entity) return notDocumented('No encuentro todavía una entidad o relación que responda a esa consulta.')
...
function intentFor(question, context) {
  ...
  if (/\b(pasos)\b/.test(q) && /\b(dirige|dirigidos|lleva|capataz|manda)\b/.test(q)) {
    return 'steps_by_agent'
  }

  if ((/\b(quien|capataz|lleva|dirige)\b/.test(q) && /\b(paso|palio|misterio|cristo)\b/.test(q)) || (context?.entityType === 'step' && /\b(quien|lleva|dirige|capataz)\b/.test(q))) {
    return 'personnel_of_step'
  }

  if (/\b(titulares|titular)\b/.test(q)) return 'titulars_of_brotherhood'

  if (/\b(pasos)\b/.test(q) && !/\b(dirige|lleva|capataz|manda)\b/.test(q)) {
