...
export async function getHistoricalMusicByBrotherhoodSlug(slug) {
  ...
  // Preview has only the publishable key. Keep the public historical timeline
  // available there; the private client in Production also resolves archived bands.
  const hasPrivateKey = Boolean(process.env.SUPABASE_SECRET_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY)
  const bandClient = hasPrivateKey ? createAdminClient() : supabase
  ...
  const [bandEntities, bandProfiles, stepEntities] = await Promise.all([
    bandIds.length
      ? bandClient
          ...
          .in('id', bandIds)
          .in('status', hasPrivateKey ? ['published', 'archived'] : ['published'])
