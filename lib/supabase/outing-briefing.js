...
async function loadOutingBriefing(outingId) {
...
export const getOutingBriefing = cache((outingId) => {
  if (!outingId) return { schedule: [], bands: [], liturgicalMusic: [], places: [] }

  return getCachedPublicData({
    key: ['outing-briefing', outingId],
    tags: [
      PUBLIC_CACHE_TAGS.OUTINGS,
      PUBLIC_CACHE_TAGS.BANDS,
      publicEntityTag('outing', outingId),
    ],
    loader: () => loadOutingBriefing(outingId),
  })
})
