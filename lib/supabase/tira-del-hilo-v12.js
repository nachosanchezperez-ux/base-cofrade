...
async function executeFreeCompoundPlan(question, context) {
  ...
  for (const query of plan.queries) {
    const response = await askHiloCofradeV11(query, activeContext)
...
export async function askHiloCofradeV12(question, context = null) {
  ...
  // encadenando varios turnos del motor relacional. El contexto de cada paso
  // alimenta al siguiente, igual que ocurriría si el usuario preguntara turno a turno.
  const freeCompound = await executeFreeCompoundPlan(clean, context)
  if (freeCompound) return freeCompound

  // Las consultas complejas que no cubre el planificador gratuito pueden pasar
  // por la capa semántica solo cuando existe opt-in explícito. Si no, V11 sigue intacto.
