...
function withExternalLinks(response) {
  ...
  return {
    ...response,
    items: response.items.map((item) => item?.externalUrl && !item?.href
      ? { ...item, href: item.externalUrl, external: true }
      : item),
  }
}
...
export async function askHiloCofradeV12(question, context = null) {
  ...
  // Si se piden las marchas de una cruceta, bajamos al repertorio completo antes
  // de que la búsqueda genérica de crucetas reduzca la respuesta a una ficha.
  ...
  // Datos directos de Marchas (datación, dedicatoria, estreno y tipo) conservan
  // prioridad porque ya adjuntan sus fuentes exactas cuando existen.
  const freeDirect = await askHiloCofradeFreeDirect(clean, context)
  if (freeDirect) return withExternalLinks(freeDirect)

  // Una Marcha puede tirar del hilo hacia sus crucetas, Bandas y otras obras del
  // mismo compositor sin duplicar la capa canónica de datos directos.
  ...
  // Las relaciones inversas de Banda necesitan reconocer también verbos naturales
  // como “acompaña” o “toca”, además de sostener repreguntas históricas con contexto.
  const bandLinksV2 = await askHiloCofradeBandLinksV2(clean, context)
  if (bandLinksV2) return withExternalLinks(bandLinksV2)

  // Relaciones de segundo nivel que necesitan cruzar varias piezas canónicas:
  // sujeto concreto -> música, históricos y filtros sobre conjuntos.
  ...
  // Colecciones públicas que ya viven en la plataforma: Agenda, Cultos,
  // Glorias, extraordinarias y crucetas. Se consultan desde sus directorios
  // y tablas canónicas para que el buscador crezca al ritmo del contenido.
