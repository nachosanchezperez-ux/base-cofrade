...
function universalHref(entity, event = null) {
  if (!entity?.slug) return ''
  if (entity.entity_type === 'image') return `/imagenes/${entity.slug}`
  if (entity.entity_type === 'step') return `/pasos/${entity.slug}`
  if (entity.entity_type === 'event' && event?.event_category === 'crew_call') return `/igualas-y-ensayos/${entity.slug}`
  return ''
}
...
export async function getGlobalSearchItems() {
  ...
  try {
    ...
    const eventIds = relatedEntities.filter((item) => item.entity_type === 'event').map((item) => item.id)
    ...
    const crewEvents = assertResult(crewEventsResult, 'No se pudieron consultar las igualás y ensayos')
    ...
    const crewEventById = new Map(crewEvents.map((item) => [item.entity_id, item]))
    ...
    const universalItems = relatedEntities.map((entity) => {
      ...
      if (entity.entity_type === 'event') {
        const event = crewEventById.get(entity.id)
        return {
          entityId: entity.id,
          entityType: entity.entity_type,
          type: event ? 'Igualá / ensayo' : TYPE_LABELS.event,
          title: entity.name,
          subtitle: event ? subtitle([event.event_type, event.event_date]) : compact(entity.summary, 90) || TYPE_LABELS.event,
          href: universalHref(entity, event),
          keywords: [entity.summary, event?.event_type, event?.event_date].filter(Boolean),
        }
      }
