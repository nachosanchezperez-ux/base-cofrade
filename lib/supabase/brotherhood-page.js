...
export async function getHermandadPageBySlug(slug) {
  ...
  try {
    enriched = await filterFutureCurrentAccompaniments(enriched)
  } catch (error) {
    console.error('[Hilo Cofrade] No se pudo acotar la vigencia temporal del acompañamiento musical', {
      slug,
      error: error instanceof Error ? error.message : String(error),
    })
  }
  ...
  try {
    enriched = await separateCouncilViaCrucis(enriched)
  } catch (error) {
    console.error('[Hilo Cofrade] No se pudo separar la participación en el Vía Crucis de las Cofradías', {
      slug,
      error: error instanceof Error ? error.message : String(error),
    })
  }
  ...
  try {
    enriched = await filterInheritedAgentSources(enriched)
  } catch (error) {
    console.error('[Hilo Cofrade] No se pudieron acotar las fuentes profesionales heredadas', {
      slug,
      error: error instanceof Error ? error.message : String(error),
    })
  }
