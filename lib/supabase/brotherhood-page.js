import 'server-only'

import { unstable_cache } from 'next/cache'
import { futureMusicAccompanimentPeriodIds } from '@/lib/music-accompaniment-effective'
import { uniquePublicOutings, publicOutingRouteSlug } from '@/lib/outings/public-outing-link'
import { getHermandadPageBySlug as getBrotherhoodDisplayBySlug } from '@/lib/supabase/brotherhood-display'
import { enrichBrotherhoodEditorialSections } from '@/lib/supabase/brotherhood-editorial-sections'
import { filterInheritedAgentSources } from '@/lib/supabase/brotherhood-inherited-source-filter'
import { getExtraordinaryGuideSlugs } from '@/lib/supabase/extraordinary-guide-slugs'
import { resolveHiloMediaReference } from '@/lib/supabase/hilo-media-paths'
import { createPublicClient as createClient } from '@/lib/supabase/public'
import { enrichBrotherhoodVisualSections } from '@/lib/supabase/brotherhood-visual-sections'

const UUID_PATTERN = /^[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}$/i

async function enrichBrotherhoodIdentityLightColor(hermandad) {
  if (!hermandad?.id) return hermandad

  const supabase = await createClient()
  const result = await supabase
    .from('brotherhood_colors')
    .select('hex_value')
    .eq('brotherhood_entity_id', hermandad.id)
    .eq('color_role', 'identity')
    .eq('status', 'published')
    .not('hex_value', 'is', null)
    .order('sort_order')
    .limit(1)
    .maybeSingle()

  if (result.error) {
    throw new Error(`No se pudo consultar el color claro de identidad: ${result.error.message}`)
  }

  const lightColor = result.data?.hex_value
  if (!lightColor) return hermandad

  return {
    ...hermandad,
    colores: {
      ...(hermandad.colores || {}),
      claro: lightColor,
    },
  }
}

async function filterFutureCurrentAccompaniments(hermandad) {
  if (!hermandad?.id) return hermandad

  const currentItems = hermandad.acompanamientoActual || []
  const periodIds = [...new Set(
    currentItems
      .map((item) => String(item?.id || '').trim())
      .filter((id) => UUID_PATTERN.test(id))
  )]
  if (!periodIds.length) return hermandad

  const supabase = await createClient()
  const result = await supabase
    .from('music_accompaniment_periods')
    .select('id, step_entity_id, date_from, year_from')
    .in('id', periodIds)
    .eq('brotherhood_entity_id', hermandad.id)
    .eq('is_current', true)
    .eq('status', 'published')

  if (result.error) {
    throw new Error(`No se pudo comprobar la vigencia temporal de los acompañamientos: ${result.error.message}`)
  }

  const futureIds = futureMusicAccompanimentPeriodIds(result.data || [])
  if (!futureIds.size) return hermandad

  const effectiveItems = currentItems.filter((item) => !futureIds.has(item.id))
  const stepIdsWithDatabaseAccompaniment = new Set(
    currentItems.map((item) => item.pasoId).filter(Boolean)
  )
  const effectiveBandsByStep = new Map()

  for (const item of effectiveItems) {
    if (!item.pasoId || !item.banda) continue
    const bands = effectiveBandsByStep.get(item.pasoId) || []
    if (!bands.includes(item.banda)) bands.push(item.banda)
    effectiveBandsByStep.set(item.pasoId, bands)
  }

  return {
    ...hermandad,
    acompanamientoActual: effectiveItems,
    pasos: (hermandad.pasos || []).map((step) => (
      stepIdsWithDatabaseAccompaniment.has(step.id)
        ? {
            ...step,
            acompanamientoActual: (effectiveBandsByStep.get(step.id) || []).join(' · '),
          }
        : step
    )),
  }
}

function historicalAccompanimentKey(item = {}) {
  return [item.banda, item.periodo, item.paso]
    .map((value) => String(value || '').trim().toLocaleLowerCase('es'))
    .join('|')
}

function historicalPeriodLabel(period = {}) {
  if (period.year_from && period.year_to) {
    return period.year_from === period.year_to
      ? String(period.year_from)
      : `${period.year_from}–${period.year_to}`
  }
  if (period.year_to) return `Hasta ${period.year_to}`
  return [period.date_from_text, period.date_to_text].filter(Boolean).join('–')
}

async function enrichBrotherhoodHistoricalAccompaniments(hermandad) {
  if (!hermandad?.id) return hermandad

  const supabase = await createClient()
  const periodsResult = await supabase
    .from('music_accompaniment_periods')
    .select('id, band_entity_id, step_entity_id, position, outing_type, date_from_text, year_from, date_to_text, year_to, notes, public_step_name')
    .eq('brotherhood_entity_id', hermandad.id)
    .eq('is_current', false)
    .eq('status', 'published')
    .order('year_to', { ascending: false, nullsFirst: false })
    .order('year_from', { ascending: false, nullsFirst: false })

  if (periodsResult.error) {
    throw new Error(`No se pudo consultar el histórico musical: ${periodsResult.error.message}`)
  }

  const periods = periodsResult.data || []
  if (!periods.length) return hermandad

  const bandIds = [...new Set(periods.map((period) => period.band_entity_id).filter(Boolean))]
  const [bandEntitiesResult, bandProfilesResult] = await Promise.all([
    bandIds.length
      ? supabase
          .from('entities')
          .select('id, name, slug, status')
          .in('id', bandIds)
          .eq('status', 'published')
      : Promise.resolve({ data: [], error: null }),
    bandIds.length
      ? supabase
          .from('bands')
          .select('entity_id, band_type')
          .in('entity_id', bandIds)
      : Promise.resolve({ data: [], error: null }),
  ])

  if (bandEntitiesResult.error) {
    throw new Error(`No se pudieron resolver las bandas históricas: ${bandEntitiesResult.error.message}`)
  }
  if (bandProfilesResult.error) {
    throw new Error(`No se pudieron resolver los tipos de las bandas históricas: ${bandProfilesResult.error.message}`)
  }

  const bandEntityById = new Map((bandEntitiesResult.data || []).map((band) => [band.id, band]))
  const bandProfileById = new Map((bandProfilesResult.data || []).map((band) => [band.entity_id, band]))
  const databaseItems = periods
    .map((period) => {
      const bandEntity = bandEntityById.get(period.band_entity_id)
      if (!bandEntity) return null

      const stepName = String(period.public_step_name || '').trim()
      const position = String(period.position || '').trim()
      const sameDetail = stepName && position
        && stepName.localeCompare(position, 'es', { sensitivity: 'base' }) === 0
      const stepDetail = sameDetail
        ? stepName
        : [stepName, position].filter(Boolean).join(' · ')

      return {
        id: period.id,
        banda: bandEntity.name,
        bandaSlug: bandEntity.slug || '',
        tipo: bandProfileById.get(period.band_entity_id)?.band_type || 'Formación musical',
        paso: stepDetail,
        periodo: historicalPeriodLabel(period),
        salida: period.outing_type || '',
        observaciones: period.notes || '',
      }
    })
    .filter(Boolean)

  const databaseKeys = new Set(databaseItems.map(historicalAccompanimentKey))
  const preservedItems = (hermandad.acompanamientos || []).filter((item) => (
    !databaseKeys.has(historicalAccompanimentKey(item))
  ))

  return {
    ...hermandad,
    acompanamientos: [...databaseItems, ...preservedItems],
  }
}

async function enrichBrotherhoodOutingImages(hermandad) {
  const outingIds = [...new Set(
    (hermandad?.salidas || [])
      .map((outing) => String(outing?.id || '').trim())
      .filter((id) => UUID_PATTERN.test(id))
  )]
  if (!outingIds.length) return hermandad

  const supabase = await createClient()
  const result = await supabase
    .from('outings')
    .select('id, hero_image_path, hero_image_alt, hero_image_credit')
    .in('id', outingIds)
    .eq('brotherhood_entity_id', hermandad.id)
    .eq('status', 'published')

  if (result.error) {
    throw new Error(`No se pudieron consultar las imágenes de las salidas: ${result.error.message}`)
  }

  const imageByOutingId = new Map((result.data || []).map((outing) => [outing.id, outing]))

  return {
    ...hermandad,
    salidas: (hermandad.salidas || []).map((outing) => {
      const image = imageByOutingId.get(outing.id)
      if (!image?.hero_image_path) return outing

      return {
        ...outing,
        imagen: {
          src: image.hero_image_path,
          alt: image.hero_image_alt || outing.nombre || 'Fotografía de la salida',
          credito: image.hero_image_credit || '',
        },
      }
    }),
  }
}

function normalizedInstitutionalLabel(value = '') {
  return String(value || '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLocaleLowerCase('es')
    .trim()
}

function isCouncilViaCrucis(item = {}) {
  const label = normalizedInstitutionalLabel(`${item.categoria || ''} ${item.titulo || ''}`)
  return label.includes('via crucis') && (label.includes('hermandad') || label.includes('cofradia'))
}

async function separateCouncilViaCrucis(hermandad) {
  const participations = hermandad?.participacionesConsejo || []
  const viaCrucisItems = participations.filter(isCouncilViaCrucis)
  if (!viaCrucisItems.length) {
    return {
      ...hermandad,
      viaCrucisCofradias: [],
    }
  }

  const eventIds = viaCrucisItems
    .map((item) => String(item.id || '').trim())
    .filter((id) => UUID_PATTERN.test(id))
  const supabase = await createClient()
  const mediaResult = eventIds.length
    ? await supabase
        .from('entity_media')
        .select('entity_id, relation_type, sort_order, is_cover, media_assets(storage_path, alt_text, author_name)')
        .in('entity_id', eventIds)
        .order('sort_order')
    : { data: [], error: null }

  if (mediaResult.error) {
    throw new Error(`No se pudo consultar la fotografía del Vía Crucis institucional: ${mediaResult.error.message}`)
  }

  const mediaByEventId = new Map()
  for (const link of mediaResult.data || []) {
    const current = mediaByEventId.get(link.entity_id)
    const asset = Array.isArray(link.media_assets) ? link.media_assets[0] : link.media_assets
    if (!asset?.storage_path) continue
    if (!current || link.is_cover || link.relation_type === 'cover') {
      mediaByEventId.set(link.entity_id, asset)
    }
  }

  return {
    ...hermandad,
    cronologia: (hermandad.cronologia || []).filter((item) => !isCouncilViaCrucis({
      categoria: item.estado,
      titulo: item.titulo,
    })),
    participacionesConsejo: participations.filter((item) => !isCouncilViaCrucis(item)),
    viaCrucisCofradias: viaCrucisItems.map((item) => {
      const asset = mediaByEventId.get(item.id)
      return {
        ...item,
        imagen: asset?.storage_path
          ? {
              src: resolveHiloMediaReference(supabase, asset.storage_path),
              alt: asset.alt_text || item.titulo || 'Vía Crucis de las Cofradías',
              credito: asset.author_name || '',
            }
          : item.imagen
            ? {
                src: item.imagen,
                alt: item.titulo || 'Vía Crucis de las Cofradías',
                credito: item.imagenCredito || '',
              }
            : null,
      }
    }),
  }
}

async function optionalBrotherhoodEnrichment(slug, label, loader, hermandad) {
  try {
    return await loader(hermandad)
  } catch (error) {
    console.error(`[Hilo Cofrade] ${label}`, {
      slug,
      error: error instanceof Error ? error.message : String(error),
    })
    return hermandad
  }
}

async function loadHermandadPageBySlug(slug) {
  const hermandad = await getBrotherhoodDisplayBySlug(slug)
  if (!hermandad) return hermandad

  const [identity, currentMusic, historicalMusic, visual, editorial, outingImages, councilViaCrucis] = await Promise.all([
    optionalBrotherhoodEnrichment(
      slug,
      'No se pudo completar el color claro de identidad de la hermandad',
      enrichBrotherhoodIdentityLightColor,
      hermandad
    ),
    optionalBrotherhoodEnrichment(
      slug,
      'No se pudo acotar la vigencia temporal del acompañamiento musical',
      filterFutureCurrentAccompaniments,
      hermandad
    ),
    optionalBrotherhoodEnrichment(
      slug,
      'No se pudo completar el histórico musical de la hermandad',
      enrichBrotherhoodHistoricalAccompaniments,
      hermandad
    ),
    optionalBrotherhoodEnrichment(
      slug,
      'No se pudieron completar los módulos visuales de la hermandad',
      enrichBrotherhoodVisualSections,
      hermandad
    ),
    optionalBrotherhoodEnrichment(
      slug,
      'No se pudieron completar los módulos editoriales de la hermandad',
      enrichBrotherhoodEditorialSections,
      hermandad
    ),
    optionalBrotherhoodEnrichment(
      slug,
      'No se pudieron completar las imágenes de las salidas',
      enrichBrotherhoodOutingImages,
      hermandad
    ),
    optionalBrotherhoodEnrichment(
      slug,
      'No se pudo separar la participación en el Vía Crucis de las Cofradías',
      separateCouncilViaCrucis,
      hermandad
    ),
  ])

  const enriched = {
    ...hermandad,
    colores: identity.colores,
    acompanamientoActual: currentMusic.acompanamientoActual,
    pasos: currentMusic.pasos,
    acompanamientos: historicalMusic.acompanamientos,
    cultos: visual.cultos,
    cartelesFiestas: visual.cartelesFiestas,
    simpecados: visual.simpecados,
    patrimonio: visual.patrimonio,
    curiosidades: editorial.curiosidades,
    salidas: outingImages.salidas,
    cronologia: councilViaCrucis.cronologia,
    participacionesConsejo: councilViaCrucis.participacionesConsejo,
    viaCrucisCofradias: councilViaCrucis.viaCrucisCofradias,
  }
  const salidas = uniquePublicOutings(enriched.salidas || [])
  const [sourceScoped, guideSlugs] = await Promise.all([
    optionalBrotherhoodEnrichment(
      slug,
      'No se pudieron acotar las fuentes profesionales heredadas',
      filterInheritedAgentSources,
      enriched
    ),
    getExtraordinaryGuideSlugs(salidas.map((outing) => outing.slug)).catch((error) => {
      console.error('[Hilo Cofrade] No se pudo resolver la ficha pública de las salidas', {
        slug,
        error: error instanceof Error ? error.message : String(error),
      })
      return new Set()
    }),
  ])

  return {
    ...sourceScoped,
    salidas: salidas.map((outing) => ({
      ...outing,
      slug: publicOutingRouteSlug(outing, guideSlugs),
    })),
  }
}

const getCachedHermandadPageBySlug = unstable_cache(
  loadHermandadPageBySlug,
  ['hilo-cofrade-public-brotherhood-detail-v3'],
  { revalidate: 3600, tags: ['public-brotherhood-detail'] }
)

export async function getHermandadPageBySlug(slug) {
  return getCachedHermandadPageBySlug(slug)
}
