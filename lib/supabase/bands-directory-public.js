...
function publicBandType(value) {
  const raw = String(value || '').trim()
  const normalized = slugify(raw)

  if (['cornetas-y-tambores', 'banda-de-cornetas-y-tambores'].includes(normalized)) {
    return 'Cornetas y Tambores'
  }

  if (normalized === 'capilla-musical') return 'Capilla Musical'

  return raw || 'Formación musical'
}
...
export async function getPublicBandsDirectory({ throwOnError = false } = {}) {
  try {
    ...
    return entities.map((entity) => {
      ...
      const type = publicBandType(band.band_type)
      ...
      return {
        ...
        municipality: municipality.name === 'Sevilla' ? 'Sevilla capital' : municipality.name || '',
