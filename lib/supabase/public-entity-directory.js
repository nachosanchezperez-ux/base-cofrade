...
function normalizeImage(item, brotherhood) {
  ...
  const normalizedImageType = normalizeDirectoryValue(item.type)
  ...
  return {
    ...
    mediaKind: 'photo',
    keywords: [item.summary, item.brotherhoodName, item.place, item.date, holyWeekDay, gloryMonth, ...types, ...item.authorNames].filter(Boolean),
  }
}
