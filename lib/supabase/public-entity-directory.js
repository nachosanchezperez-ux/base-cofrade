...
import { MONTHS, holyWeekDay as canonicalHolyWeekDay, normalizeDirectoryValue } from '@/lib/brotherhood-directory'
...
function processionCalendar(item) {
  ...
  const holyWeekDay = isPenitencia ? canonicalHolyWeekDay(item) : ''
...
export async function getPublicEntityDirectory({ throwOnError = false } = {}) {
  const [brotherhoods, images, steps, bands] = await Promise.all([
    getHermandadesDirectory({ throwOnError }),
    getImagesDirectory({ throwOnError }),
    getStepsDirectory({ throwOnError }),
    getPublicBandsDirectory({ throwOnError }),
  ])
