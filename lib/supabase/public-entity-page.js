...
function containsEditorialPlaceholder(value) {
  if (typeof value === 'string') return isEditorialPlaceholder(value)
  if (Array.isArray(value)) return value.some(containsEditorialPlaceholder)
  if (value && typeof value === 'object') return Object.values(value).some(containsEditorialPlaceholder)
  return false
}
...
export function meetsPublicEditorialMinimum({
  ...
  return Boolean(
    ...
    && sources.length > 0
    && !containsEditorialPlaceholder(publicValues)
  )
}
