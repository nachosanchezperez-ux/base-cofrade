...
export async function getExtraordinaryDetail(slug) {
  ...
  try {
    ...
    const [scheduleRows, musicRows, sourceLinkRows, outingMediaRows] = await Promise.all([
      assertRows(
        await supabase
          ...
          .select('id, sequence_no, label, item_date, item_time, time_text, place_id, notes')
    ...
    const [places, entities, sources] = await Promise.all([
      placeIds.length
        ? assertRows(
      ...
      sourceIds.length
        ? assertRows(
            await supabase
              .from('sources')
              .select('id, name, url, source_type, author_or_publisher, publication_date, notes')
              .in('id', sourceIds),
            'No se pudieron consultar las fuentes documentales de la extraordinaria'
          )
        : [],
    ])
    ...
    return {
      ...
      origin: outing.origin_place_name || '',
      destination: outing.destination_place_name || '',
