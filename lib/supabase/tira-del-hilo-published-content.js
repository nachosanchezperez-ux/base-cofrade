...
import {
  ...
  agendaQueryCategory,
  agendaQueryCategoryLabel,
  agendaQueryCountLabel,
  agendaQueryHref,
  agendaQueryPeriod,
  agendaQueryPeriodLabel,
} from '@/lib/tira-agenda-query'
import {
...
async function repertoiresAnswer(question) {
  ...
  const strongMatches = specificTokens
    ? scored.filter(({ score }) => score >= Math.max(200, Math.round(bestScore * 0.6)))
    : scored
  const rows = (specificTokens ? strongMatches.map(({ item }) => item) : repertoires).slice(0, 12)
  if (!rows.length) return notDocumented('Todavía no hay crucetas musicales publicadas que coincidan con esa consulta.')

  const resultLabel = rows.length === 1 ? '1 cruceta musical' : `${rows.length} crucetas musicales`

  return answer({
    text: rows.length === 1
      ? 'Hilo Cofrade tiene 1 cruceta musical publicada que coincide con la consulta.'
      : `Hilo Cofrade tiene ${rows.length} crucetas musicales publicadas que coinciden con la consulta.`,
    ...
    context: resultSetContext(rows.map((item) => item.brotherhood?.id), resultLabel, 'published_musical_repertoires'),
  })
}
...
async function agendaAnswer(question) {
  ...
  const category = agendaQueryCategory(question)
  const period = agendaQueryPeriod(question)
  const matched = items.filter((item) => agendaItemMatchesQuery(item, {
    today,
    municipality,
    category,
    period,
  ...
  const categoryLabel = agendaQueryCategoryLabel(category)
  const periodLabel = agendaQueryPeriodLabel(period)
  const agendaHref = agendaQueryHref({ category, period, municipality })

  if (!rows.length) {
    const categoryText = categoryLabel ? ` de ${categoryLabel.toLocaleLowerCase('es')}` : ''
    const periodText = period === 'upcoming' ? 'entre los próximos actos' : `para ${periodLabel}`
    return notDocumented(`No hay resultados publicados${categoryText} en ${scope} ${periodText}.`)
  }

  const resultLabel = agendaQueryCountLabel(category, matched.length)
  const periodText = period === 'upcoming' ? 'entre los próximos actos' : `para ${periodLabel}`

  return answer({
    text: `La Agenda Cofrade tiene ${matched.length} ${resultLabel} en ${scope} ${periodText}.`,
    path: ['Agenda Cofrade', categoryLabel || 'Próximos actos', scope, periodLabel],
    items: rows.map((item) => ({
      label: item.title,
      meta: [item.categoryLabel, item.dateInfo?.label, item.startTime, item.municipality, item.organizer].filter(Boolean).join(' · '),
      href: item.href || item.categoryHref || agendaHref,
      group: item.categoryName,
    })),
    links: [{ label: 'Abrir esta selección en la Agenda Cofrade', href: agendaHref }],
  })
}
