...
function scheduleReturnTime(rows = []) {
  ...
  return timeLabel(entry?.item_time)
}
...
export async function getExtraordinaryDirectory() {
  try {
    ...
    const [musicRows, scheduleRows] = outingIds.length
      ? await Promise.all([
          ...
          supabase
            ...
            .select('outing_id, sequence_no, label, item_time')
            .in('outing_id', outingIds)
            .order('sequence_no'),
    ...
    return rows.map((item) => {
      ...
      const fallbackReturnTime = scheduleReturnTime(scheduleByOuting.get(item.id) || [])
      ...
      return {
        ...
        returnTime: timeLabel(item.return_time) || fallbackReturnTime,
        reason: item.reason || '',
