...
export async function getExtraordinaryDirectory({ throwOnError = false } = {}) {
  try {
    ...
    const [musicRows, scheduleRows, entities, brotherhoodRows] = await Promise.all([
      outingIds.length
        ? supabase
      ...
      brotherhoodIds.length
        ? supabase
          .from('entities')
          .select('id, slug, entity_type, status')
      ...
      brotherhoodIds.length
        ...
        : Promise.resolve({ data: [], error: null }),
    ]).then(([musicResult, scheduleResult, entityResult, brotherhoodResult]) => [
      ...
      assertRows(entityResult, 'No se pudieron consultar las Hermandades de las extraordinarias'),
      assertRows(brotherhoodResult, 'No se pudieron consultar los escudos de las extraordinarias'),
    ])
    ...
    return rows.map((item) => {
      ...
      return {
        ...
        brotherhoodHref: publicBrotherhoodHref(entityById.get(item.brotherhood_entity_id)),
        crestPath: crestByEntityId.get(item.brotherhood_entity_id) || '',
        municipality,
