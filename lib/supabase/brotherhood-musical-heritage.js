...
function recordingUrl(recording) {
  ...
  if (recording.youtube_video_id) {
    return `https://www.youtube.com/watch?v=${recording.youtube_video_id}`;
  }
  return '';
}
...
export async function getBrotherhoodMusicalHeritage(brotherhoodEntityId) {
  ...
  const dedicationRows = assertQuery(
    dedicationResult,
    'No se pudo consultar el patrimonio musical de la hermandad'
  );
  const currentMusicRows = assertQuery(
    currentMusicResult,
    'No se pudo consultar el contexto musical actual de la hermandad'
  );
  const heritageRows = assertQuery(
    heritageResult,
    'No se pudieron consultar las composiciones musicales patrimoniales'
  );
  ...
  if (!marchIds.length && !heritageIds.length) return [];
  ...
  if (marchIds.length) {
    const [entityResult, marchResult, authorResult, recordingResult, trackResult] = await Promise.all([
      ...
      supabase
        .from('marches')
        .select('entity_id, composition_year, composition_date_text, music_type, youtube_video_id, premiere_date, premiere_date_text, premiered_by_band_entity_id')
        .in('entity_id', marchIds),
      supabase
        .from('march_authors')
        .select('id, march_entity_id, agent_entity_id, author_role, notes')
        .in('march_entity_id', marchIds)
        .eq('status', 'published'),
      supabase
        .from('march_recordings')
        .select('id, march_entity_id, band_entity_id, youtube_video_id, external_url, title, is_featured, status')
        .in('march_entity_id', marchIds)
        .eq('status', 'published'),
      supabase
        .from('band_release_tracks')
        .select('id, release_id, march_entity_id, spotify_url')
        .in('march_entity_id', marchIds),
    ]);

    const marchEntities = assertQuery(entityResult, 'No se pudieron consultar las marchas publicadas');
    const marchRows = assertQuery(marchResult, 'No se pudieron consultar los datos musicales');
    const authorRows = assertQuery(authorResult, 'No se pudieron consultar las autorías de las marchas');
    const recordingRows = assertQuery(recordingResult, 'No se pudieron consultar las grabaciones de las marchas');
    const trackRows = assertQuery(trackResult, 'No se pudieron consultar las pistas discográficas de las marchas');
    ...
    const releaseRows = releaseIds.length
      ? assertQuery(
          await supabase
            .from('band_releases')
            .select('id, band_entity_id, title, status')
            .in('id', releaseIds)
            .eq('status', 'published'),
          'No se pudieron consultar los lanzamientos discográficos'
        )
      : [];
    ...
    const [relatedEntities, bandRows] = await Promise.all([
      relatedIds.length
        ? supabase
            ...
            .in('id', relatedIds)
            .eq('status', 'published')
            .then((result) => assertQuery(result, 'No se pudieron consultar autores y bandas relacionadas'))
        : Promise.resolve([]),
      bandIds.length
        ? supabase
            .from('bands')
            ...
            .then((result) => assertQuery(result, 'No se pudieron consultar los tipos de formación musical'))
        : Promise.resolve([]),
    ]);
    ...
    marchItems = marchEntities.map((entity) => {
      ...
      const authors = authorRows
        ...
        .map((item) => ({
          ...
          notes: item.notes || '',
          entity: relatedById.get(item.agent_entity_id),
        }))
        .filter((item) => item.entity?.name);
      const composers = authors.filter((item) => item.role === 'composer');
      const adapters = authors.filter((item) => item.role === 'adapter');
      ...
      const discographyTrack = tracks.find(
        (item) => item.spotify_url && releaseById.get(item.release_id)
      ) || tracks.find((item) => releaseById.get(item.release_id)) || tracks[0] || null;
      ...
      const bandRelation = premiereBand
        ...
        : recordingBand
          ...
          : releaseBand
            ? { label: 'Grabada por', name: releaseBand.name, slug: releaseBand.slug }
            : null;

      const featuredUrl = recordingUrl(featuredRecording);
      const listeningUrl = featuredUrl
        || discographyTrack?.spotify_url
        || (march.youtube_video_id ? `https://www.youtube.com/watch?v=${march.youtube_video_id}` : '');
      ...
      return {
        ...
        musicType: march.music_type || 'Marcha procesional',
        composers: composers.map((item) => ({
        ...
        listening: listeningUrl
          ? { url: listeningUrl, provider: listeningProvider(listeningUrl) }
          : null,
      };
    });
  }
  ...
  if (heritageIds.length) {
    const [heritageEntityResult, heritageAuthorResult] = await Promise.all([
      ...
      supabase
        ...
        .select('id, source_entity_id, target_entity_id, notes')
        .in('target_entity_id', heritageIds)
        .eq('relation_type', 'author_of')
        .eq('status', 'published'),
    ]);

    const heritageEntities = assertQuery(
      heritageEntityResult,
      'No se pudieron consultar las obras musicales patrimoniales'
    );
    const heritageAuthorRows = assertQuery(
      heritageAuthorResult,
      'No se pudieron consultar las autorías del patrimonio musical no procesional'
    );
    ...
    const heritageAgents = heritageAgentIds.length
      ? assertQuery(
          await supabase
            .from('entities')
            .select('id, name, slug, status')
            .in('id', heritageAgentIds)
            .eq('status', 'published'),
          'No se pudieron consultar los autores de capilla y coplas'
        )
      : [];
    ...
    heritageItems = heritageEntities.map((entity) => {
      ...
      const authors = heritageAuthorRows
        ...
        .map((item) => ({
          entity: agentById.get(item.source_entity_id),
          label: heritageRoleLabel(item.notes),
        }))
        .filter((item) => item.entity?.name);
      const musicAuthors = authors.filter((item) => item.label !== 'Letra');
      const lyricists = authors.filter((item) => item.label === 'Letra');
      ...
      return {
        ...
        composers: musicAuthors.map((item) => ({
          name: item.entity.name,
          slug: item.entity.slug,
        })),
        adapters: lyricists.map((item) => ({
          name: item.entity.name,
          slug: item.entity.slug,
          label: 'Letra',
        })),
  ...
  return [...marchItems, ...heritageItems].sort((a, b) => {
    const yearA = Number.parseInt(a.year, 10) || 0;
    const yearB = Number.parseInt(b.year, 10) || 0;
    return yearB - yearA || a.name.localeCompare(b.name, 'es');
  });
}
