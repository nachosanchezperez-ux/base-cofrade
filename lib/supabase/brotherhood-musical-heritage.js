...
export async function getBrotherhoodMusicalHeritage(
  brotherhoodEntityId,
  { imageIds = [], currentAccompaniments = [] } = {}
) {
  if (!brotherhoodEntityId) return [];

  const supabase = await createClient();
  const suppliedImageIds = unique(imageIds);
  const resolvedImageIds = suppliedImageIds.length
    ? suppliedImageIds
    : assertQuery(
        await supabase
          ...
          .eq('status', 'published'),
        'No se pudieron consultar las imágenes vinculadas a la hermandad'
      ).map((item) => item.image_entity_id);
  const dedicateeIds = unique([
    brotherhoodEntityId,
    ...resolvedImageIds,
  ]);
  const suppliedCurrentMusicRows = currentAccompaniments
    .filter((item) => item?.bandaId)
    .map((item) => ({
      band_entity_id: item.bandaId,
  ...
  const [dedicationResult, currentMusicResult, heritageResult] = await Promise.all([
    supabase
      ...
      .eq('status', 'published'),
    suppliedCurrentMusicRows.length
      ? Promise.resolve({ data: suppliedCurrentMusicRows, error: null })
      : supabase
  ...
  if (heritageIds.length) {
    ...
    heritageItems = heritageEntities.map((entity) => {
      ...
      return {
        ...
        workType: asset.asset_type === 'Copla' ? 'Copla' : 'Música para cultos',
