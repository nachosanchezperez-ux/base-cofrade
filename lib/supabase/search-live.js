...
export async function searchHiloEntities(rawTerm, requestedLimit = 8) {
  ...
  const [brotherhoodRowsResult, bandRowsResult, imageRowsResult, stepRowsResult, disciplineResult, marchAuthorsResult, coverRelationsResult] = await Promise.all([
  ...
  const enrichmentFailure = [brotherhoodRowsResult, bandRowsResult, imageRowsResult, stepRowsResult, disciplineResult, marchAuthorsResult, coverRelationsResult].find((result) => result.error)
