...
export async function searchHiloEntities(rawTerm, requestedLimit = 8) {
  ...
  const [
    ...
    optionalRows(supabase.from('entities').select('id, entity_type, name, slug, summary, status').in('entity_type', SEARCHABLE_TYPES).eq('status', 'published').ilike('name', pattern).limit(candidateLimit), 'entity_names', failures),
    optionalRows(supabase.from('entities').select('id, entity_type, name, slug, summary, status').in('entity_type', SEARCHABLE_TYPES).eq('status', 'published').ilike('summary', pattern).limit(candidateLimit), 'entity_summaries', failures),
