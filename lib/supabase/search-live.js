...
async function loadSearchSupportSnapshot() {
  ...
  const [
    ...
    supabase.from('brotherhoods').select('entity_id, popular_name, official_name'),
    supabase.from('band_names').select('band_entity_id, name, short_name'),
    supabase.from('entity_names').select('entity_id, name, status'),
    supabase.from('image_names').select('image_entity_id, name'),
    supabase.from('agent_names').select('agent_entity_id, name'),
    supabase.from('municipalities').select('id, name').order('name'),
  ])

  const entries = [
    ['brotherhoods', brotherhoodsResult],
    ['band_names', bandNamesResult],
    ['entity_names', entityNamesResult],
    ['image_names', imageNamesResult],
    ['agent_names', agentNamesResult],
    ['municipalities', municipalitiesResult],
  ]
  for (const [label, result] of entries) {
    if (result.error) throw new Error(`No se pudo cargar el soporte de búsqueda ${label}: ${result.error.message}`)
  }

  return {
    brotherhoods: brotherhoodsResult.data || [],
    bandNames: bandNamesResult.data || [],
    entityNames: entityNamesResult.data || [],
    imageNames: imageNamesResult.data || [],
    agentNames: agentNamesResult.data || [],
    municipalities: municipalitiesResult.data || [],
  }
}

const getCachedSearchSupportSnapshot = unstable_cache(
  loadSearchSupportSnapshot,
  ['hilo-cofrade-search-support-v1'],
  { revalidate: 60 }
)

function searchSupportMatch(value, term) {
  const text = String(value || '').toLocaleLowerCase('es-ES')
  const query = String(term || '').toLocaleLowerCase('es-ES')
  return Boolean(query) && text.includes(query)
}

async function getSearchSupportRows(supabase, term, pattern, candidateLimit, failures = []) {
  try {
    const support = await getCachedSearchSupportSnapshot()
    return {
      brotherhoodPopularRows: support.brotherhoods
        .filter((row) => searchSupportMatch(row.popular_name, term))
        .slice(0, candidateLimit),
      brotherhoodOfficialRows: support.brotherhoods
        .filter((row) => searchSupportMatch(row.official_name, term))
        .slice(0, candidateLimit),
      bandNameRows: support.bandNames
        .filter((row) => searchSupportMatch(row.name, term))
        .slice(0, candidateLimit),
      bandShortNameRows: support.bandNames
        .filter((row) => searchSupportMatch(row.short_name, term))
        .slice(0, candidateLimit),
      entityAliasRows: support.entityNames
        .filter((row) => row.status === 'published' && searchSupportMatch(row.name, term))
        .slice(0, candidateLimit),
      imageAliasRows: support.imageNames
        .filter((row) => searchSupportMatch(row.name, term))
        .slice(0, candidateLimit),
      agentAliasRows: support.agentNames
        .filter((row) => searchSupportMatch(row.name, term))
        .slice(0, candidateLimit),
    ...
    failures.push({
      stage: 'search_support_snapshot',
      code: error?.name || 'request_failed',
    })
  }
...
function assertCompleteSearch(failures = [], throwOnPartial = false) {
  if (!throwOnPartial || !failures.length) return

  const error = new Error('El autocompletado no se completó con todas sus fuentes')
  error.name = 'PartialHiloSearchError'
  error.stages = [...new Set(failures.map((failure) => failure.stage))]
  error.codes = [...new Set(failures.map((failure) => failure.code))]
  throw error
}
...
async function loadPublishedContentSearchUncached(supabase, term, candidateLimit, failures = []) {
  const pattern = `%${term}%`
...
async function loadPublishedContentSearchCached(term, candidateLimit) {
  const supabase = await createClient()
  const failures = []
  const items = await loadPublishedContentSearchUncached(
    supabase,
    term,
    candidateLimit,
    failures
  ...
  if (failures.length) {
    const error = new Error('No se pudo completar el snapshot de contenidos del buscador')
    error.name = 'SearchContentCacheLoadError'
    throw error
  }
...
const getCachedPublishedContentSearch = unstable_cache(
  loadPublishedContentSearchCached,
  ['hilo-cofrade-search-content-v1'],
  { revalidate: 30 }
)

async function searchPublishedContent(supabase, term, candidateLimit, failures = []) {
  try {
    return await getCachedPublishedContentSearch(term, candidateLimit)
  } catch (error) {
    failures.push({
      stage: 'content_search_cache',
      code: error?.name || 'request_failed',
    })
    return loadPublishedContentSearchUncached(
      supabase,
      term,
      candidateLimit,
      failures
...
export async function searchHiloEntities(rawTerm, requestedLimit = 8, { throwOnPartial = false } = {}) {
  ...
  const [
    ...
    getSearchSupportRows(supabase, term, pattern, candidateLimit, failures),
  ...
  if (!candidates.length) {
    ...
    assertCompleteSearch(failures, throwOnPartial)
