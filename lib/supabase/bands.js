  ...
  curiosities: [
    {
      ...
      title: '¿Sabías que…?',
      summary: 'Las Cigarreras acompañó al Cristo de la Salud de la Hermandad de San Bernardo desde 1993 hasta 2003. De esta vinculación nace la marcha «Refúgiame», de Francis González Ríos.',
      body: 'Las Cigarreras acompañó al Cristo de la Salud de la Hermandad de San Bernardo desde 1993 hasta 2003. De esta vinculación nace la marcha «Refúgiame», de Francis González Ríos.',
    },
  ],
  outings: [
    {
...
async function loadBandRelations(supabase, band) {
  ...
  return {
    ...
    historicalAccompaniments: periodRows.filter((item) => !item.is_current).map(serializePeriod),
    curiosities: curiosityRows.map((item) => ({
      id: item.id,
      title: item.title,
