...
async function loadBandsDirectory() {
  try {
...
export const getBandsDirectory = cache(() => getCachedPublicData({
  key: ['band-directory'],
  tags: [PUBLIC_CACHE_TAGS.BANDS, PUBLIC_CACHE_TAGS.BROTHERHOODS],
  loader: loadBandsDirectory,
}))

export const getBandBySlug = cache((slug) => getCachedPublicData({
  key: ['band-page', slug],
  tags: [
    ...
    publicEntityTag('band', slug),
  ],
  loader: () => loadBandBySlug(slug),
}))
