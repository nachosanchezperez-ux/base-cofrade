...
async function loadBandRelations(supabase, band) {
  ...
  const publicPeriodResult = await supabase.rpc('get_public_band_accompaniments', { target_band_id: band.id })
  const publicPeriodRows = publicPeriodResult.error ? [] : publicPeriodResult.data || []
  ...
  return {
    ...
    accompaniments: periodRows.map((item) => {
      const publicItem = publicPeriodRows.find((row) => row.id === item.id)
      ...
      return {
        id: item.id,
        brotherhoodName: publicItem?.brotherhood_name || brotherhood?.name || 'Hermandad por documentar',
        brotherhoodSlug: publicItem?.brotherhood_slug || (brotherhood?.status === 'published' ? brotherhood.slug || '' : ''),
        stepName: publicItem?.step_name || step?.name || '',
