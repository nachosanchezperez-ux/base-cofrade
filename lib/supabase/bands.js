...
const FALLBACK_BAND = {
  ...
  summary: 'Banda sevillana de cornetas y tambores vinculada a la Hermandad de la Columna y Azotes.',
  ...
  linkedBrotherhood: 'Hermandad de la Columna y Azotes (Las Cigarreras)',
...
async function loadCore(supabase, entities) {
  ...
  const [bands, names] = await Promise.all([
  ...
  return entities.map((entity) => {
    ...
    return {
      ...
      linkedBrotherhood: band.linked_brotherhood_name || '',
  ...
  return {
    ...
    outings: outings.map((outing) => {
      ...
      return {
        ...
        position: position?.position_label || position?.position_code || '',
      }
    }),
  }
}
...
export async function getBandBySlug(slug) {
  try {
    ...
    if (!band) return null
    return { ...band, ...(await loadBandRelations(supabase, band)) }
  } catch (error) {
