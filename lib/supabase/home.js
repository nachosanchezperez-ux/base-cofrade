...
import { isEditorialHomeThread } from '@/lib/home-discovery-editorial'
...
export async function getTodayHomeContent() {
  ...
  try {
    ...
    try {
      ...
      if (!marchEntityId) {
        const marchCandidates = await getDailyMarchCandidates(supabase, today)
        marchEntityId = chooseCandidate(marchCandidates, `${today}:march`)?.entity_id || null
      }
      march = await marchCard(supabase, marchEntityId)
    } catch (error) {
...
export async function getHomeDiscoveryThreads(limit = 3) {
  try {
    ...
    const select = 'thread_key, root_entity_id, root_type, root_name, root_slug, activity_kind, relation_count, secondary_count, latest_at, priority'
    let rows = assertQuery(
      await supabase
    ...
    if (rows.length < limit) {
      rows = assertQuery(
        await supabase
          ...
          .order('priority', { ascending: false })
          .limit(fetchLimit),
        'No se pudo ampliar la actividad de conocimiento de la Home'
      )
    }
    ...
    for (const row of rows) {
      if (!isEditorialHomeThread(row)) continue
  ...
  try {
    ...
    const rows = assertQuery(
      await supabase
        ...
        .gte('outing_date', previousDateKey(today))
