...
import { buildSourceLinkOrFilters } from '@/lib/brotherhood-source-scope'
...
async function loadBrotherhoodFromSupabase(slug) {
  ...
  const sourceLinkFilters = buildSourceLinkOrFilters([
    ['entity_id', sourceTargetIds],
    ...sourceLinkTargets,
  ])
  ...
  // Agrupar objetivos en OR permite que Postgres combine los índices existentes
  // mediante BitmapOr y evita repetir la planificación RLS por cada columna.
  try {
    for (const filter of sourceLinkFilters) {
