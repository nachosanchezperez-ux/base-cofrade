...
import { formatCultOccurrenceDate, formatCultOccurrenceSchedule, nextCultOccurrence } from '@/lib/cults/occurrences'
...
function isMissingPublicSupabaseConfig(error) {
  const message = error instanceof Error ? error.message : String(error || '')
  return /faltan? (?:las variables públicas|la configuración pública) de supabase/i.test(message)
}
...
async function loadBrotherhoodFromSupabase(slug) {
  ...
  const entity = assertQuery(
    await supabase
      .from('entities')
      .select('id, entity_type, name, slug, summary, status, updated_at, content_updated_at, editorial_reviewed_at')
  ...
  const [
    ...
    supabase
      ...
      .select('id, name, tunic_description, hood_description, cord_description, buttons_description, shield_description, footwear_description, gloves_color, image_path, image_alt, sort_order')
      .eq('brotherhood_entity_id', entity.id)
      .eq('status', 'published')
      .order('sort_order'),
    supabase
    ...
    supabase
      .from('outings')
      .select('id, slug, outing_type, character, title, outing_date, return_date, year, departure_time, return_time, municipality_id, origin_place_id, destination_place_id, reason, route, route_summary, description, public_notes, event_status, outing_series_id')
      .eq('brotherhood_entity_id', entity.id)
      .eq('status', 'published')
    ...
    supabase
      ...
      .select('id, year, total_nazarenos_count, total_procession_count, position_by_nazarenos, position_by_procession, brotherhoods_in_day, official_career_duration_minutes, members_count, members_count_kind, source_id')
      .eq('brotherhood_entity_id', entity.id)
      .eq('status', 'published')
      .order('year', { ascending: false })
      .limit(1)
  ...
  const cultOccurrenceRows = cultRows.length
    ? assertQuery(
        await supabase
          .from('cult_occurrences')
          .select('id, cult_id, year, start_date, end_date, place_id, event_status')
          .in('cult_id', cultRows.map((cult) => cult.id))
          .eq('status', 'published')
          .neq('event_status', 'cancelled')
          .gte('end_date', today)
          .order('start_date'),
        'No se pudieron consultar las próximas ediciones de los cultos'
      )
    : []
  const cultOccurrenceIds = cultOccurrenceRows.map((occurrence) => occurrence.id)
  const cultPlaceIds = unique([
    ...cultRows.map((cult) => cult.place_id),
    ...cultOccurrenceRows.map((occurrence) => occurrence.place_id),
  ])
  const [cultOccurrenceDaysResult, cultPlacesResult] = await Promise.all([
    cultOccurrenceIds.length
      ? supabase
          .from('cult_occurrence_days')
          .select('cult_occurrence_id, day_number, celebration_date, start_time, time_text')
          .in('cult_occurrence_id', cultOccurrenceIds)
          .order('celebration_date')
      : Promise.resolve({ data: [], error: null }),
    cultPlaceIds.length
      ? supabase.from('places').select('id, name').in('id', cultPlaceIds)
      : Promise.resolve({ data: [], error: null }),
  ])
  const cultOccurrenceDays = assertQuery(cultOccurrenceDaysResult, 'No se pudieron consultar los horarios de los cultos')
  const cultPlaces = assertQuery(cultPlacesResult, 'No se pudieron consultar los lugares de los cultos')
  ...
  // Las fuentes propias de los agentes son globales y no pertenecen a la ficha de
  // la Hermandad salvo que también estén vinculadas a una pieza o fase visible.
  // Evitar esos IDs mantiene acotada la consulta pública más costosa.
  const sourceTargetIds = unique([entity.id, ...imageIds, ...stepIds, ...heritageAssetIds, ...eventIds])
  const sourceLinks = []
  const sourceLinkTargets = [
    ['heritage_update_id', heritageUpdateIds],
    ['intervention_id', heritageInterventionIds],
    ['step_phase_id', stepPhaseIds],
    ['cult_id', cultRows.map((item) => item.id)],
    ['outing_id', outingRows.map((item) => item.id)],
    ['outing_series_id', outingSeriesRows.map((item) => item.id)],
    ['music_accompaniment_period_id', musicPeriodIds],
  ]

  // Las fuentes enriquecen la ficha, pero no forman parte de su identidad. Un
  // timeout en este bloque nunca debe convertir una Hermandad publicada en 404.
  try {
    if (sourceTargetIds.length) {
      sourceLinks.push(...await loadPublicRowsInBatches(
        sourceTargetIds,
        (ids) => supabase.from('source_links').select('source_id').in('entity_id', ids),
        'No se pudieron consultar los enlaces de fuentes',
        { attempts: 1, batchSize: 24 }
      ))
    }

    // Estas consultas son muy pequeñas, pero lanzarlas todas a la vez multiplica
    // los picos de conexiones cuando varias fichas se regeneran simultáneamente.
    for (const [column, ids] of sourceLinkTargets) {
      if (!ids.length) continue
      sourceLinks.push(...assertQuery(
        await supabase.from('source_links').select('source_id').in(column, ids),
        'No se pudieron consultar los enlaces de fuentes'
      ))
    }
  } catch (error) {
    sourceLinks.length = 0
    console.error('[Hilo Cofrade] Fuentes de la hermandad omitidas temporalmente', {
      slug,
      error,
    })
  }
  ...
  if (sourceIds.length) {
    ...
    if (sourcesResult.error) {
      console.error('[Hilo Cofrade] Fuentes documentales de la hermandad omitidas temporalmente', {
        slug,
  ...
  const cultPlaceById = new Map(cultPlaces.map((place) => [place.id, place.name]))
  const cultos = cultRows.map((cult) => {
    const occurrence = nextCultOccurrence(cultOccurrenceRows, cult.id)
    const days = occurrence
      ? cultOccurrenceDays.filter((day) => day.cult_occurrence_id === occurrence.id)
      : []
    const schedule = formatCultOccurrenceSchedule(days) || cult.time_text
    const recurringDate = cultDate(cult)

    return {
      id: cult.id,
      nombre: cult.title,
      tipo: [cult.cult_type, schedule].filter(Boolean).join(' · '),
      descripcion: cult.description || '',
      ...recurringDate,
      ...(occurrence ? {
        fechaCorta: formatCultOccurrenceDate(occurrence.start_date, occurrence.end_date),
        fechaDetalle: `EDICIÓN ${occurrence.year}`,
        lugar: cultPlaceById.get(occurrence.place_id || cult.place_id) || '',
        anunciada: occurrence.event_status === 'announced',
      } : {}),
    }
  })
  ...
  const habitos = habitRows.map((habit) => ({
    ...
    guantes: habit.gloves_color === 'Blanco'
      ? 'Blancos'
      : habit.gloves_color === 'Negro' ? 'Negros' : '',
    imagenPath: habit.image_path || '',
    imagenAlt: habit.image_alt || '',
  }))

  const outingSeriesById = new Map(outingSeriesRows.map((outing) => [outing.id, outing]))
  const salidasConcretas = outingRows.map((outing) => {
    ...
    return {
      id: outing.id,
      serieId: outing.outing_series_id || '',
      diaLiturgico: outingSeriesById.get(outing.outing_series_id)?.date_rule || '',
      slug: outing.slug || '',
  ...
  const datosJornada = processionStats
    ? {
        ...
        totalHermanos: processionStats.members_count?.toLocaleString('es-ES') || '',
        membersCount: processionStats.members_count ?? null,
        membersCountKind: processionStats.members_count_kind || '',
...
function mergeBrotherhood(local, remote) {
  ...
  return {
    ...
    updatedAt: remote.entity.content_updated_at || remote.entity.updated_at || null,
...
export async function getHermandadPageBySlug(slug) {
  ...
  try {
    ...
    if (local || isMissingPublicSupabaseConfig(error)) return local
    throw error
  }
}
