...
async function loadBrotherhoodFromSupabase(slug) {
  ...
  const acompanamientoActual = musicPeriods.map((period) => {
    ...
    return {
      ...
      banda: bandEntity?.name || 'Banda por documentar',
  ...
  const esCartelDeFiestas = (pieza) => {
    const tipo = normalized(pieza.tipo)
    return tipo.includes('cartel de las fiestas') || tipo.includes('cartel anunciador de las fiestas')
  }
  const cartelesFiestas = piezasPatrimoniales
    .filter(esCartelDeFiestas)
    .sort((first, second) => yearFrom(second.fecha) - yearFrom(first.fecha))
  const patrimonio = piezasPatrimoniales.filter((pieza) => !esCartelDeFiestas(pieza))
