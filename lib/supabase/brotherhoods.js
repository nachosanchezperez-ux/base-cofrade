...
import {
  getHermandadesDirectory as getBaseHermandadesDirectory,
  getHermandadPageBySlug as getBaseHermandadPageBySlug,
  getImagenPageBySlug as getBaseImagenPageBySlug,
  getPasoPageBySlug as getBasePasoPageBySlug,
} from '@/lib/supabase/brotherhoods-base'
...
export async function getHermandadesDirectory() {
  return getBaseHermandadesDirectory()
}
...
export async function getImagenPageBySlug(slug) {
  const base = await getBaseImagenPageBySlug(slug)
  if (!base?.hermandad) return base

  const hermandad = await getHermandadPageBySlug(base.hermandad.slug)
  if (!hermandad) return base

  const imagen = hermandad.imagenes.find((item) => (
    item.id === base.imagen?.id || item.slug === slug
  )) || base.imagen

  return { imagen, hermandad }
}

export async function getPasoPageBySlug(slug) {
  const base = await getBasePasoPageBySlug(slug)
  if (!base?.hermandad) return base

  const hermandad = await getHermandadPageBySlug(base.hermandad.slug)
  if (!hermandad) return base

  const paso = hermandad.pasos.find((item) => (
    item.id === base.paso?.id || item.slug === slug
  )) || base.paso

  return { paso, hermandad }
}
