...
async function loadHermandadesDirectory() {
  try {
    const supabase = createPublicClient()
...
export const getHermandadesDirectory = cache(() => getCachedPublicData({
  key: ['legacy-brotherhood-directory'],
  tags: [PUBLIC_CACHE_TAGS.BROTHERHOODS],
  loader: loadHermandadesDirectory,
}))

export const getHermandadPageBySlug = cache((slug) => getCachedPublicData({
  key: ['brotherhood-page', slug],
  tags: [
    ...
    PUBLIC_CACHE_TAGS.SOURCES,
    publicEntityTag('brotherhood', slug),
  ],
  loader: () => loadHermandadPageBySlug(slug),
}))

export const getImagenPageBySlug = cache((slug) => getCachedPublicData({
  key: ['legacy-image-page', slug],
  tags: [PUBLIC_CACHE_TAGS.IMAGES, PUBLIC_CACHE_TAGS.BROTHERHOODS],
  loader: () => loadImagenPageBySlug(slug),
}))

export const getPasoPageBySlug = cache((slug) => getCachedPublicData({
  key: ['legacy-step-page', slug],
  tags: [PUBLIC_CACHE_TAGS.STEPS, PUBLIC_CACHE_TAGS.BROTHERHOODS],
  loader: () => loadPasoPageBySlug(slug),
}))
