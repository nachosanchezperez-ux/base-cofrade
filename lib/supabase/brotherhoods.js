...
async function loadBrotherhoodFromSupabase(slug) {
  ...
  const outingSeriesMovementRows = outingSeriesRows.length
    ...
    : []
  const outingEntityLinks = outingRows.length
    ? assertQuery(
        await supabase
        ...
        'No se pudieron consultar las imágenes que participan en las salidas'
      )
    : []
  ...
  const salidasConcretas = outingRows.map((outing) => ({
    ...
    titulares: outingEntityLinks
      ...
      .filter(Boolean)
      .join(' · '),
    momento: [displayDate(outing.outing_date), documentedPublicText(outing.description)].filter(Boolean).join(' · '),
