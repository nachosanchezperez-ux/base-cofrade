...
async function loadBrotherhoodFromSupabase(slug) {
  ...
  const sourceTargetIds = unique([entity.id, ...imageIds, ...stepIds, ...eventIds, ...agentIds])
...
export async function getHermandadesDirectory() {
  try {
    ...
    if (!entities.length) return hermandades

    const entityIds = entities.map((entity) => entity.id)
    const brotherhoodRows = assertQuery(
      await supabase
        .from('brotherhoods')
        .select('entity_id, official_name, popular_name, municipality_id, canonical_see_place_id, crest_path, brotherhood_types, current_procession_day')
    ...
    return mergeBySlug(hermandades, remoteItems)
  } catch (error) {
