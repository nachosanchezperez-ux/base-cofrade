...
async function loadBrotherhoodFromSupabase(slug) {
  ...
  const [
    ...
    supabase
      .from('outings')
      .select('id, slug, outing_type, character, title, outing_date, return_date, year, departure_time, return_time, municipality_id, origin_place_id, destination_place_id, reason, route, route_summary, description, public_notes, event_status')
      .eq('brotherhood_entity_id', entity.id)
      .eq('status', 'published')
