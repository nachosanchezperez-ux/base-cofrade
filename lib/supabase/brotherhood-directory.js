...
export async function getHermandadesDirectory() {
  try {
    ...
    const [brotherhoodRows, glorySeriesRows] = await Promise.all([
      supabase
        .from('brotherhoods')
    ...
    const publishedBrotherhoodRows = assertRows(
      brotherhoodRows,
      'No se pudieron consultar los datos del directorio de Hermandades'
    )
    const publishedGlorySeriesRows = assertRows(
      glorySeriesRows,
      'No se pudieron consultar las fechas principales de las Glorias'
    )
    const municipalityIds = unique(publishedBrotherhoodRows.map((row) => row.municipality_id))
    const placeIds = unique(publishedBrotherhoodRows.map((row) => row.canonical_see_place_id))
    ...
    const brotherhoodById = new Map(publishedBrotherhoodRows.map((row) => [row.entity_id, row]))
    ...
    for (const series of publishedGlorySeriesRows) {
    ...
    return filterPublicPageEntities(entities, publishedBrotherhoodRows).map((entity) => {
