...
import { mergeBandInterestLinks } from '@/lib/bands/official-links'
import { presentBandPremiere } from '@/lib/bands/premieres'
...
async function loadCore(supabase, entities) {
  ...
  return entities.map((entity) => {
    ...
    return {
      ...
      interestLinks: mergeBandInterestLinks(
        socialLinkRows
...
export async function getBandsDirectory() {
  try {
    ...
    const entities = assertQuery(
      await supabase
        .from('entities')
        .select('id, name, slug, summary, updated_at, content_updated_at, editorial_reviewed_at')
        .eq('entity_type', 'band')
        .eq('status', 'published')
        .order('name'),
...
async function loadBandRelations(supabase, band) {
  ...
  const municipalities = municipalityIds.length
    ...
    : []
  const municipalityById = byId(municipalities)
  const outingBrotherhoodIds = unique(outings.map((item) => item.brotherhood_entity_id))
  const outingBrotherhoods = outingBrotherhoodIds.length
    ? assertQuery(
        await supabase
          .from('entities')
          .select('id, slug, status')
          .eq('entity_type', 'brotherhood')
          .in('id', outingBrotherhoodIds),
        'No se pudieron resolver las Hermandades de las próximas salidas'
      )
    : []
  const outingBrotherhoodById = byId(outingBrotherhoods)
  ...
  return {
    ...
    premieres: premiereRows.map((item) => {
      ...
      const presentation = presentBandPremiere(item.description)
    ...
    heritage: heritageRows
      .map((item) => {
        ...
        return {
          ...
          description: entity.summary || item.description || '',
    ...
    outings: outings.map((outing) => {
      ...
      const municipality = municipalityById.get(outing.municipality_id) || {}
      return {
        id: outing.id,
        slug: outing.slug || '',
        title: outing.title || outing.outing_type,
        type: outing.outing_type,
        character: outing.character || '',
        brotherhoodEntityId: outing.brotherhood_entity_id || '',
        brotherhoodSlug: outingBrotherhoodById.get(outing.brotherhood_entity_id)?.status === 'published'
          ? outingBrotherhoodById.get(outing.brotherhood_entity_id)?.slug || ''
          : '',
        date: outing.outing_date,
        year: Number(outing.year) || Number(String(outing.outing_date || '').slice(0, 4)) || null,
