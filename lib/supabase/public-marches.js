...
function authorHref(entity, role = '') {
  if (role !== 'composer' || !entity?.slug || entity.status !== 'published' || entity.entity_type !== 'agent') return ''
  return `/autores/${entity.slug}`
}
...
export const getPublicMarchSitemapEntries = cache(async function getPublicMarchSitemapEntries() {
  ...
  const rows = await loadPublicRowsInPages(
    (from, to) => supabase
      .from('entities')
      .select('id, slug, updated_at, content_updated_at')
  ...
  return rows
    ...
    .map((item) => ({
      id: item.id,
      slug: item.slug,
      updatedAt: item.content_updated_at || item.updated_at || null,
    }))
})
...
export const getPublicMarchDirectory = cache(async function getPublicMarchDirectory() {
  ...
  const entities = await loadPublicRowsInPages(
    (from, to) => supabase
      .from('entities')
      .select('id, name, slug, summary, updated_at, content_updated_at')
      .eq('entity_type', 'march')
      .eq('status', 'published')
      .not('slug', 'is', null)
      .order('name')
...
export const getPublicMarchAuthorBySlug = cache(async function getPublicMarchAuthorBySlug(slug) {
  if (!slug) return null
  ...
  if (agentResult.error) throw new Error(`No se pudo consultar el compositor: ${agentResult.error.message}`)
  if (!agentResult.data) return null

  const agent = agentResult.data
  const authorships = rows(
    await supabase
      .from('march_authors')
      ...
      .eq('status', 'published'),
    'No se pudieron consultar las obras del compositor'
  )

  if (!authorships.length) {
    return { id: agent.id, slug: agent.slug, name: agent.name, summary: agent.summary || '', marches: [] }
  }

  const marchIds = unique(authorships.map((item) => item.march_entity_id))
  const directory = await getPublicMarchDirectory()
  const marchIdSet = new Set(marchIds)
  ...
  return {
    ...
    marches: directory.filter((march) => marchIdSet.has(march.id)),
  }
})
...
async function loadPublicMarchBySlug(slug) {
  try {
    ...
    const [repertoiresResult, releasesResult] = await Promise.all([
      repertoireIds.length
        ? supabase
            ...
            .select('id, slug, title, outing_id, band_entity_id, step_entity_id, repertoire_kind, status')
            .in('id', repertoireIds)
    ...
    const relatedEntityIds = unique([
      ...
      ...repertoires.map((item) => item.step_entity_id),
    ...
    const [brotherhoodsResult, relatedEntitiesResult, authorProfilesResult] = await Promise.all([
      ...
      authors.length
        ? supabase
            ...
            .in('entity_id', unique(authors.map((item) => item.agent_entity_id)))
        : Promise.resolve({ data: [], error: null }),
    ])
    ...
    const authorProfiles = rows(authorProfilesResult, 'No se pudieron cargar los tipos de Autor')
    ...
    const authorProfileById = new Map(authorProfiles.map((item) => [item.entity_id, item]))
    ...
    const repertoireHistory = entries
      .map((entry) => {
        ...
        const step = relatedById.get(repertoire.step_entity_id)
        ...
        return {
          ...
          step: step ? { name: step.name, href: entityHref(step) } : null,
    ...
    const discographyHistory = releases
      .map((release) => {
        const band = relatedById.get(release.band_entity_id)
        const bandHref = entityHref(band)
        const track = tracks.find((item) => item.release_id === release.id)
        return {
          id: release.id,
          title: release.title,
          year: release.release_year,
          band: band ? { name: band.name, href: bandHref } : null,
          bandDiscographyHref: bandHref ? `${bandHref}#discografia` : '',
    ...
    return {
      ...
      authors: authors.map((item) => {
        const author = relatedById.get(item.agent_entity_id)
        return {
          id: item.agent_entity_id,
          name: author?.name || '',
          role: item.author_role || '',
          notes: item.notes || '',
          kind: authorProfileById.get(item.agent_entity_id)?.agent_kind || '',
          href: authorHref(author, item.author_role || ''),
        }
...
const getCachedPublicMarchBySlug = unstable_cache(
  ...
  ['hilo-cofrade-public-march-detail-v4'],
