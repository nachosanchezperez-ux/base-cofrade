...
async function loadRelationalPresence(items = []) {
...
const getCachedRelationalPresence = unstable_cache(
  loadRelationalPresence,
  ['hilo-cofrade-public-relational-presence-v2'],
  { revalidate: 900, tags: ['public-relational-presence'] }
);

export async function enrichRelationalPresence(items = []) {
  return getCachedRelationalPresence(items);
}
