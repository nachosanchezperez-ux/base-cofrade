...
export async function getPublishedEntityCoverMediaMap(entityIds = [], { entityTypesById = null } = {}) {
  const ids = [...new Set(entityIds.filter(Boolean))]
  if (!ids.length) return new Map()

  try {
    const supabase = createPublicClient()
    const knownEntityTypes = entityTypesById instanceof Map
      ? entityTypesById
      : new Map(Object.entries(entityTypesById || {}))
    const needsEntityTypeLookup = ids.some((id) => !knownEntityTypes.has(id))

    const [entities, relations] = await Promise.all([
      needsEntityTypeLookup
        ? loadPublicRowsInBatches(
            ids.filter((id) => !knownEntityTypes.has(id)),
            (batchIds) => supabase
              .from('entities')
              .select('id, entity_type')
              .in('id', batchIds),
            'No se pudieron consultar los tipos de las entidades con portada'
          )
        : [],
      loadPublicRowsInBatches(
        ids,
        ...
        'No se pudieron consultar las relaciones de portada'
      ),
    ])
    ...
    const entityTypeById = new Map([
      ...knownEntityTypes,
      ...entities.map((entity) => [entity.id, entity.entity_type]),
    ])
    ...
    const mediaRows = await loadPublicRowsInBatches(
      mediaIds,
      (batchIds) => supabase
        .from('media_assets')
      ...
      'No se pudieron consultar los recursos de portada'
    )
