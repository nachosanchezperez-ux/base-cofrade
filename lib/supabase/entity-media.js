...
function serializeCoverMedia(media, publicPath = '') {
...
export async function getPublishedEntityCoverMediaMap(entityIds = []) {
  ...
  try {
    ...
    relations.forEach((relation) => {
      ...
      const serialized = serializeCoverMedia(media, resolvePublicMediaPath(supabase, media?.storage_path))
...
export async function getPublishedEntityMedia(entityId) {
  ...
  try {
    ...
    return relations.map((relation) => {
      const media = mediaById.get(relation.media_asset_id)
      const serialized = serializeCoverMedia(media, resolvePublicMediaPath(supabase, media?.storage_path))
      return serialized ? { ...serialized, id: relation.id, relationType: relation.relation_type, sortOrder: relation.sort_order, isCover: relation.is_cover, notes: relation.notes || '' } : null
    }).filter(Boolean)
  } catch (error) {
