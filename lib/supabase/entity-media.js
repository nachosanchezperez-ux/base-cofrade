...
export async function getPublishedEntityCoverMediaMap(entityIds = []) {
  ...
  try {
    ...
    const { data: relations, error: relationError } = await supabase
      .from('entity_media')
      .select('entity_id, media_asset_id, sort_order')
      .in('entity_id', ids)
      .eq('is_cover', true)
    ...
    const { data: mediaRows, error: mediaError } = await supabase
      .from('media_assets')
      .select('id, storage_path, title, caption, alt_text, author_name, source_name, source_url, rights_status')
      .in('id', mediaIds)
      .eq('media_type', 'image')
    ...
    relations.forEach((relation) => {
      ...
      const media = serializeCoverMedia(mediaById.get(relation.media_asset_id))
...
export async function getPublishedEntityMedia(entityId) {
  ...
  try {
    ...
    const { data: relations, error: relationError } = await supabase
      .from('entity_media')
      .select('id, media_asset_id, relation_type, sort_order, is_cover, notes')
