...
function serializeCoverMedia(media, relation = {}, publicPath = '') {
  ...
  return {
    ...
    focusX: Number(relation.focus_x ?? 50),
    focusY: Number(relation.focus_y ?? 50),
    mobileFocusX: Number(relation.mobile_focus_x ?? relation.focus_x ?? 50),
    mobileFocusY: Number(relation.mobile_focus_y ?? relation.focus_y ?? 50),
    fitMode: relation.fit_mode || 'auto',
    focusPosition: `${Number(relation.focus_x ?? 50)}% ${Number(relation.focus_y ?? 50)}%`,
  }
}
...
export async function getPublishedEntityCoverMediaMap(entityIds = []) {
  ...
  try {
    ...
    const { data: mediaRows, error: mediaError } = await supabase
      .from('media_assets')
      .select('id, storage_path, title, caption, alt_text, author_name, source_name, source_url, rights_status, width_px, height_px')
      .in('id', mediaIds)
      .eq('media_type', 'image')
    ...
    relations.forEach((relation) => {
      if (covers.has(relation.entity_id)) return
      const mediaRow = mediaById.get(relation.media_asset_id)
      const media = serializeCoverMedia(
        mediaRow,
        relation,
        resolvePublicMediaPath(supabase, mediaRow?.storage_path)
      )
      if (media) covers.set(relation.entity_id, media)
    })
