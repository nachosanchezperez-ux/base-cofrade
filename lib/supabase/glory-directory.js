...
async function loadGloryDirectory(slug = '') {
    ...
    return rows
      .map((row) => {
        ...
        return {
          ...
          brotherhoodName: brotherhood?.name || 'Hermandad por confirmar',
...
const getCachedGloryDirectory = unstable_cache(
  loadGloryDirectory,
  ['hilo-cofrade-public-glory-directory-v1'],
  { revalidate: 300, tags: ['public-glory-directory'] }
)

export async function getGloryDirectory({ throwOnError = false, slug = '' } = {}) {
  try {
    return await getCachedGloryDirectory(slug)
  } catch (error) {
...
async function loadGloryDetail(slug) {
  if (!slug) return null

  try {
    const targetedDirectory = await getGloryDirectory({ throwOnError: true, slug })
    let item = targetedDirectory[0] || null

    // Conserva compatibilidad con salidas antiguas cuyo slug público se deriva
    // editorialmente porque la columna outings.slug está vacía.
    if (!item) {
      const directory = await getGloryDirectory({ throwOnError: true })
      item = directory.find((outing) => outing.slug === slug) || null
    }

    if (!item) return null

    const supabase = createPublicClient()
    const [outingResult, scheduleResult, musicResult, sourceLinksResult] = await Promise.all([
      supabase
        .from('outings')
    ...
    const scheduleRows = assertRows(scheduleResult, 'No se pudieron consultar los horarios de la procesión de Gloria')
    const musicRows = assertRows(musicResult, 'No se pudo consultar la música de la procesión de Gloria')
    const sourceLinkRows = sourceLinksResult.error ? [] : sourceLinksResult.data || []
    if (sourceLinksResult.error) {
      console.error('[Hilo Cofrade] Fuentes de la procesión de Gloria omitidas temporalmente', {
        slug,
        error: sourceLinksResult.error,
      })
    }
    ...
    const [sourcesResult, bandsResult] = await Promise.all([
      sourceIds.length
        ? supabase
            .from('sources')
    ...
    if (sourcesResult.error) {
      console.error('[Hilo Cofrade] Fuentes documentales de la procesión de Gloria omitidas temporalmente', {
        slug,
        error: sourcesResult.error,
      })
    }
    const bands = assertRows(bandsResult, 'No se pudieron consultar las bandas relacionadas con la procesión de Gloria')
    ...
    const music = musicRows.map((row) => {
      ...
      return {
        ...
        name: row.band_name || band?.name || 'Acompañamiento musical por confirmar',
    ...
    return {
      ...
      schedule,
      music,
      processionalMusic: music.filter((row) => ['processional_music', 'procession', 'opening', 'behind_step', 'behind_glory'].includes(row.type)),
...
const getCachedGloryDetail = unstable_cache(
  loadGloryDetail,
  ['hilo-cofrade-public-glory-detail-v1'],
  { revalidate: 300, tags: ['public-glory-detail'] }
)

export async function getGloryDetail(slug) {
  if (!slug) return null
  return getCachedGloryDetail(slug)
}
