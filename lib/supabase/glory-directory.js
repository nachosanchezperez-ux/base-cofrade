...
async function loadGloryDirectory(slug = '') {
  ...
  return rows
    .map((row) => {
      ...
      const resolvedSlug = fallbackSlug(row, brotherhood)
      ...
      return {
        id: row.id,
        slug: resolvedSlug,
        detailHref: `/procesiones-de-gloria/${resolvedSlug}`,
...
export async function getGloryDirectory({ throwOnError = false, slug = '' } = {}) {
  try {
    return await loadGloryDirectory(slug)
  } catch (error) {
...
async function loadGloryDetail(slug) {
  ...
  try {
    ...
    // Conserva compatibilidad con salidas históricas cuyo slug público se deriva
    // porque la columna outings.slug puede estar vacía.
    if (!item) {
...
const getCachedGloryDetail = unstable_cache(
  loadGloryDetail,
  ['hilo-cofrade-public-glory-detail-v2'],
