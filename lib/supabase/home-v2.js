...
export async function getHomeExploreStats() {
  ...
  try {
    const supabase = await createClient()
    const [directoryItems, graphCounts] = await Promise.all([
      getPublicEntityDirectory(),
      Promise.all(GRAPH_TYPES.map((entityType) => (
        supabase
          .from('entities')
    ...
    const counts = new Map(GRAPH_TYPES.map((entityType, index) => [
      entityType,
      assertCount(graphCounts[index], `No se pudo contar ${entityType}`),
    ]))
