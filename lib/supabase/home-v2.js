...
export async function getHomeExploreStats() {
  ...
  try {
    const supabase = await createClient()
    const [directoryItems, graphRows] = await Promise.all([
      getPublicEntityDirectory(),
      supabase
        .from('entities')
        .select('entity_type')
        .in('entity_type', ['march', 'agent', 'heritage_asset', 'event'])
        .eq('status', 'published'),
    ])
    const rows = assertQuery(graphRows, 'No se pudieron consultar las magnitudes públicas del grafo')
