...
import { getDailyMarchCandidates } from '@/lib/supabase/daily-march-candidates'

const NAVIGABLE_TYPES = ['brotherhood', 'image', 'step', 'band']
const GRAPH_TYPES = ['march', 'agent', 'heritage_asset', 'event']
...
const DISCOVERY_FAST_PATH_PRIORITY = 92
const DISCOVERY_SELECT = 'thread_key, root_entity_id, root_type, root_name, root_slug, activity_kind, relation_count, secondary_count, priority'
...
function assertCount(result, label) {
  if (result.error) throw new Error(`${label}: ${result.error.message}`)
...
function agree(count, singular, pluralForm) {
  return count === 1 ? singular : pluralForm
}

function featureCardOrNull(card) {
  if (!card) return null
  return publicText(card.title) && publicText(card.summary) && card.href ? card : null
}
...
function discoveryCardFromRow(row) {
  ...
  if (row.activity_kind === 'musical_heritage') {
    const total = count + secondary
    return {
      ...base,
      kicker: 'Hermandad → patrimonio musical',
      summary: count && secondary
        ? `${total} composiciones documentadas ${agree(total, 'permite', 'permiten')} recorrer marchas, piezas, autores y grabaciones desde la Hermandad.`
  ...
  if (row.activity_kind === 'posters') {
    return {
      ...
      summary: `${plural(count, 'cartel', 'carteles')} ${agree(count, 'forma', 'forman')} una puerta de entrada a su memoria gráfica y a las autorías documentadas.`,
  ...
  if (row.activity_kind === 'step_personnel') {
    return {
      ...
      summary: `${plural(count, 'paso', 'pasos')} ${agree(count, 'queda conectado', 'quedan conectados')} con ${plural(secondary, 'capataz', 'capataces')} para recorrer sus responsables actuales.`,
      href: rootHref(row),
  ...
  if (row.activity_kind === 'titularity') {
    return {
      ...
      summary: `${plural(count, 'titular', 'titulares')} ${agree(count, 'permite', 'permiten')} continuar hacia imágenes, advocaciones y presencia procesional sin mezclar conceptos distintos.`,
  ...
  if (row.activity_kind === 'brotherhood_steps') {
    return {
      ...
      summary: `${plural(count, 'paso', 'pasos')} ${agree(count, 'conecta', 'conectan')} la Hermandad con patrimonio, imágenes, autores, capataces y acompañamientos.`,
      href: rootHref(row),
  ...
  if (row.activity_kind === 'image_authorship') {
    return {
      ...
      summary: `${plural(count, 'autor', 'autores')} ${agree(count, 'conecta', 'conectan')} la imagen con su creación y con el resto de obras documentadas de sus responsables.`,
      href: rootHref(row),
  ...
  if (row.activity_kind === 'step_phases') {
    return {
      ...
      summary: `${plural(count, 'fase', 'fases')} ${agree(count, 'permite', 'permiten')} entender el paso como un conjunto que ha evolucionado con el tiempo.`,
      href: rootHref(row),
...
function rankDiscovery(rows, blockedRoots) {
  return rows
...
function chooseDiscovery(rows, today, blockedRoots) {
  const candidates = rankDiscovery(rows, blockedRoots).slice(0, DISCOVERY_POOL_SIZE)
...
async function marchCard(supabase, marchEntityId) {
  ...
  const [authorsResult, dedicationsResult, recordingsResult, tracksResult] = await Promise.all([
    ...
    supabase
      ...
      .select('youtube_video_id, external_url, band_entity_id, title, is_featured')
      .eq('march_entity_id', marchEntityId)
      .eq('status', 'published')
  ...
  if (tracks.length) {
    ...
    const releases = releaseIds.length
      ? assertQuery(
          await supabase
            .from('band_releases')
            .select('id, band_entity_id, title, cover_image_path, cover_image_alt')
            .in('id', releaseIds)
            .eq('status', 'published'),
    ...
    const sameRecordingBand = (item) => {
      const itemRelease = releaseById.get(item.release_id)
      return itemRelease && recording?.band_entity_id && itemRelease.band_entity_id === recording.band_entity_id
    }
    const exactRecordingTrack = tracks.find((item) => (
      recording?.external_url
      && item.spotify_url === recording.external_url
      && releaseById.has(item.release_id)
    ))
    const featuredTrack = exactRecordingTrack
      || tracks.find((item) => item.spotify_url && sameRecordingBand(item))
      || tracks.find((item) => sameRecordingBand(item))
      || tracks.find((item) => item.spotify_url && releaseById.has(item.release_id))
      || tracks.find((item) => releaseById.has(item.release_id))
      || null
    release = featuredTrack ? releaseById.get(featuredTrack.release_id) || null : null
    spotifyUrl = featuredTrack?.spotify_url || ''
  }

  const bandEntityId = recording?.band_entity_id || release?.band_entity_id || null
  const [composer, dedicatee, band] = await Promise.all([
    composerLink?.agent_entity_id ? entityById(supabase, composerLink.agent_entity_id) : Promise.resolve(null),
    dedication?.dedicatee_entity_id ? entityById(supabase, dedication.dedicatee_entity_id) : Promise.resolve(null),
    bandEntityId ? entityById(supabase, bandEntityId) : Promise.resolve(null),
  ])
  ...
  return {
    id: entity.id,
    title: entity.name,
    composer: publicText(composer?.name),
    year: march.composition_year || march.composition_date_text || '',
    dedicatee: dedicatee?.name || dedication?.dedication_text || '',
    performedBy: band?.name || '',
    bandHref: entityHref(band),
    releaseTitle: release?.title || recording?.title || '',
    releaseHref: band?.slug && release ? `/bandas/${band.slug}#discografia` : '',
    coverImagePath: release?.cover_image_path || '',
    coverImageAlt: release?.cover_image_alt || (release?.title ? `Portada de ${release.title}` : ''),
    videoId,
    listenUrl,
    whyToday,
  }
...
export async function getTodayHomeContentV2() {
  ...
  try {
    ...
    const [overridesResult, ephemerisResult, editorialResult, discoveryResult] = await Promise.all([
      supabase
      ...
      supabase
        ...
        .in('activity_kind', [...DISCOVERY_KINDS])
        .gte('priority', DISCOVERY_FAST_PATH_PRIORITY),
    ])
    ...
    const editorialOverride = overrides.find((item) => item.content_type === 'curiosity') || null
    const factOverride = overrides.find((item) => item.content_type === 'fact') || null
    let editorial = null
    let fact = null
    if (editorialOverride) {
      editorial = await editorialOverrideCard(supabase, editorialOverride)
    } else {
      const selectedEditorial = chooseCandidate(
    ...
    if (factOverride) {
      fact = await editorialOverrideCard(supabase, factOverride)
    } else {
      const selectedFact = chooseCandidate(
        editorialCandidates.filter((item) => item.content_type === 'fact'),
        `${today}:fact`
      )
      fact = await editorialCard(supabase, selectedFact, 'fact')
    }
    ...
    let discoveryPool = discoveryRows
    if (rankDiscovery(discoveryPool, blockedRoots).length < DISCOVERY_POOL_SIZE) {
      discoveryPool = assertQuery(
        await supabase
          .from('home_knowledge_threads')
          .select(DISCOVERY_SELECT)
          .in('activity_kind', [...DISCOVERY_KINDS]),
        'No se pudieron ampliar los hilos candidatos del día'
      )
    }
    const selectedDiscovery = chooseDiscovery(discoveryPool, today, blockedRoots)
    const discovery = discoveryCardFromRow(selectedDiscovery)

    let publicMarch = null
    try {
      const marchOverride = overrides.find((item) => item.content_type === 'march') || null
      const marchCandidates = marchOverride
        ? []
        : await getDailyMarchCandidates(supabase, today)
      ...
      publicMarch = march
        && publicText(march.title)
        && publicText(march.composer)
        && (march.listenUrl || march.releaseHref || march.bandHref)
        ? march
        : null
    } catch (error) {
      console.error('[Hilo Cofrade] No se pudo cargar la Marcha del día', {
        error: error instanceof Error ? error.message : String(error),
      })
    }

    return {
      ephemeris: featureCardOrNull(ephemeris),
      editorial: featureCardOrNull(editorial),
      fact: featureCardOrNull(fact),
      discovery: featureCardOrNull(discovery),
      march: publicMarch,
    }
  } catch (error) {
...
export async function getHomeExploreStats({ throwOnError = false } = {}) {
  const unavailable = {
    directories: [
      { key: 'brotherhood', label: 'Hermandades', count: null, href: '/hermandades', detail: 'Historia · titulares · cultos · pasos · música' },
      { key: 'image', label: 'Imágenes', count: null, href: '/imagenes', detail: 'Autoría · cronología · restauraciones · pasos' },
      { key: 'step', label: 'Pasos', count: null, href: '/pasos', detail: 'Patrimonio · fases · autores · capataces · música' },
      { key: 'band', label: 'Bandas', count: null, href: '/bandas', detail: 'Trayectoria · acompañamientos · discografía · relaciones' },
    ],
    graph: [
      { key: 'march', count: null, label: 'marchas documentadas' },
      { key: 'agent', count: null, label: 'autores, artistas y profesionales' },
      { key: 'heritage_asset', count: null, label: 'elementos patrimoniales' },
      { key: 'event', count: null, label: 'acontecimientos' },
    ],
  }
  ...
  try {
    ...
    const results = await Promise.allSettled([
      getPublicEntityDirectory({ throwOnError }),
      ...GRAPH_TYPES.map((entityType) => (
        supabase
          .from('entities')
    ...
    const directoryResult = results[0]

    if (directoryResult.status === 'fulfilled') {
      const directoryItems = directoryResult.value
      for (const item of directoryItems) {
        counts.set(item.kind, (counts.get(item.kind) || 0) + 1)
      }
    } else {
      console.error('[Hilo Cofrade] No se pudieron contar los directorios de la Home', {
        error: directoryResult.reason instanceof Error ? directoryResult.reason.message : String(directoryResult.reason),
      })
    }

    GRAPH_TYPES.forEach((entityType, index) => {
      const result = results[index + 1]
      if (result.status === 'rejected') {
        console.error(`[Hilo Cofrade] No se pudo contar ${entityType} en la Home`, {
      ...
      counts.set(entityType, Number(result.value.count) || 0)
    })

    return {
      directories: unavailable.directories.map((item) => ({
        ...item,
        count: counts.has(item.key) ? counts.get(item.key) : null,
      })),
      graph: unavailable.graph.map((item) => ({
        ...item,
        count: counts.has(item.key) ? counts.get(item.key) : null,
      })),
    }
