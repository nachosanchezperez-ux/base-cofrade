...
export const DEFAULT_PUBLIC_QUERY_FIRST_ATTEMPT_TIMEOUT_MS = 10_000
...
export function publicQueryFetchPolicy(env = process.env) {
  ...
  const configuredFirstAttemptMs = positiveMilliseconds(
    env.SUPABASE_PUBLIC_QUERY_FIRST_ATTEMPT_TIMEOUT_MS,
    DEFAULT_PUBLIC_QUERY_FIRST_ATTEMPT_TIMEOUT_MS
  )

  return {
    totalTimeoutMs,
    firstAttemptTimeoutMs: Math.min(configuredFirstAttemptMs, totalTimeoutMs),
    retryDelayMs: positiveMilliseconds(
...
export function createPublicQueryFetch(
  ...
  return async function fetchPublicQuery(input, init = {}) {
    ...
    while (attempts < (canRetry ? 2 : 1)) {
      ...
      const attemptTimeoutMs = attempts === 1 && canRetry
        ? Math.min(policy.firstAttemptTimeoutMs, remainingMs)
        : remainingMs
