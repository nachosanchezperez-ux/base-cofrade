...
function brotherhoodTypeFromQuestion(question) {
  const q = normalize(question)
  if (/\bagrupacion(?:es)? parroquial(?:es)?\b/.test(q)) return 'Agrupación Parroquial'
...
function multiIntent(question) {
  ...
  if (/\bbandas?\b/.test(q) && /\b(?:hermandades?|agrupaciones? parroquiales?)\b/.test(q) && (/\b(gloria|glorias|penitencia|sacramental|sacramentales|agrupacion(?:es)? parroquial(?:es)?)\b/.test(q) || /\ben\b/.test(q))) {
