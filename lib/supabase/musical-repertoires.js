...
export async function getMusicalRepertoires({ brotherhoodEntityId = '', bandEntityId = '', outingIds = [] } = {}) {
  try {
    const supabase = createPublicClient()
    ...
    if (brotherhoodEntityId) {
      const suppliedOutingIds = unique(outingIds)
      const resolvedOutingIds = suppliedOutingIds.length
        ? suppliedOutingIds
        : rows(
            await supabase
              .from('outings')
      ...
      if (!resolvedOutingIds.length) return []
      query = query.in('outing_id', resolvedOutingIds)
    }
