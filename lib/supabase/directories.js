...
import { isMissingPublicSupabaseConfig, publicErrorMessage } from '@/lib/supabase/public-error'
...
function publicMunicipality(value = '') { return value === 'Sevilla' ? 'Sevilla capital' : value }
function publicImageType(value = '') {
  const type = value || 'Imagen'
  const normalized = type.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[·-]+/g, ' ').replace(/\s+/g, ' ').toLowerCase().trim()
  return ['virgen gloria', 'virgen gloriosa', 'virgen de gloria'].includes(normalized) ? 'Virgen de Gloria' : type
}
...
async function loadImagesDirectory() {
  try {
    ...
    return filterPublicPageEntities(entities, images).map((entity) => {
...
async function loadStepsDirectory() {
  try {
    ...
    return filterPublicPageEntities(entities, steps).map((entity) => {
      ...
      return { id: entity.id, slug: entity.slug, name: entity.name, href: `/pasos/${entity.slug}`, summary: entity.summary || step.description || '', type: step.step_type || 'Paso procesional', style: step.style || '', materials: step.materials || '', date: step.execution_date_text || '', condition: step.current_condition || '', brotherhoodName: brotherhoodEntity?.name || brotherhoodRelation?.brotherhood_name || '', brotherhoodSlug: brotherhoodEntity?.slug || '', municipality: publicMunicipality(municipality?.name || ''), municipalitySlug: municipality?.slug || '', imageNames: unique(imageRelations.map((item) => item.image_name)), authors: agents.map((name) => ({ name })), authorNames: agents, disciplines, coverPath: coverMedia?.path || '', coverAlt: coverMedia?.alt || '', coverCredit: coverMedia?.credit || '' }
    })
  } catch (error) {
    console.error('[Hilo Cofrade] No se pudo cargar el directorio de pasos', { error: publicErrorMessage(error) })
    throw error instanceof Error ? error : new Error(publicErrorMessage(error))
  }
}

const getCachedImagesDirectory = unstable_cache(
  loadImagesDirectory,
  ['hilo-cofrade-public-images-directory-v2'],
  { revalidate: 900, tags: ['public-images-directory'] }
)

const getCachedStepsDirectory = unstable_cache(
  loadStepsDirectory,
  ['hilo-cofrade-public-steps-directory-v2'],
  { revalidate: 900, tags: ['public-steps-directory'] }
)

export async function getImagesDirectory({ throwOnError = false } = {}) {
  try {
    return await getCachedImagesDirectory()
  } catch (error) {
    if (isMissingPublicSupabaseConfig(error)) return []
    if (throwOnError) throw error
    return []
  }
}

export async function getStepsDirectory({ throwOnError = false } = {}) {
  try {
    return await getCachedStepsDirectory()
  } catch (error) {
    if (isMissingPublicSupabaseConfig(error)) return []
    if (throwOnError) throw error
    return []
