...
export async function getImagesDirectory() {
  try {
    ...
    const coverMediaByEntity = await getPublishedEntityCoverMediaMap(entities.map((item) => item.id))
    ...
    return entities.map((entity) => {
      ...
      const coverMedia = coverMediaByEntity.get(entity.id)
