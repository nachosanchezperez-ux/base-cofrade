...
async function loadDirectoryRows(supabase, brotherhoodId = '') {
  let query = supabase
    .from('events')
  ...
  if (brotherhoodId) query = query.eq('brotherhood_entity_id', brotherhoodId)
...
export async function getCrewEventDirectory({ brotherhoodId = '', throwOnError = false } = {}) {
  try {
    const supabase = createPublicClient()
    const today = madridDateKey()
    const events = await loadDirectoryRows(supabase, brotherhoodId)
    ...
    const [stepLinks, agentLinks] = await Promise.all([
      supabase
        .from('crew_event_steps')
        .select('id, event_entity_id, step_entity_id, is_primary, sort_order, notes')
        .in('event_entity_id', eventIds)
        .order('sort_order')
        .then((result) => assertRows(result, 'No se pudieron consultar los Pasos de las convocatorias')),
      supabase
        .from('crew_event_agents')
        .select('id, event_entity_id, agent_entity_id, role_name, is_primary, sort_order, notes')
        .in('event_entity_id', eventIds)
        .order('sort_order')
        .then((result) => assertRows(result, 'No se pudieron consultar los capataces de las convocatorias')),
    ])
    ...
    const placeIds = [...new Set(events.map((item) => item.place_id).filter(Boolean))]
...
export async function getCrewEventsByBrotherhoodId(brotherhoodId) {
  if (!brotherhoodId) return []
  return getCrewEventDirectory({ brotherhoodId })
}
