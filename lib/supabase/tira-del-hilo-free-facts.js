...
function answer({ text, path = [], entities = [], items = [], links = [], evidence = [], references = [], referencesNote = '', followUps = [], context = null, compactItemLimit = 3 }) {
  return {
    kind: 'answer',
    answer: text,
    path,
...
function brotherhoodCollectionLabel(type, count) {
  const plural = count !== 1
  if (type === 'Agrupación Parroquial') return plural ? 'agrupaciones parroquiales publicadas' : 'agrupación parroquial publicada'
  if (type === 'Penitencia') return plural ? 'hermandades de penitencia publicadas' : 'hermandad de penitencia publicada'
  if (type === 'Gloria') return plural ? 'hermandades de gloria publicadas' : 'hermandad de gloria publicada'
  if (type === 'Sacramental') return plural ? 'corporaciones sacramentales publicadas' : 'corporación sacramental publicada'
  return plural ? 'corporaciones publicadas' : 'corporación publicada'
}

function brotherhoodGroup(item) {
  return hasDirectoryType(item, 'agrupaciones-parroquiales')
    ? 'Agrupaciones parroquiales'
    : 'Hermandades'
}

async function brotherhoodsByMunicipality(question, { brotherhoodType = '', processionDay = '' } = {}) {
  const directory = await getHermandadesDirectory()
  if (!directory.length) return null

  const municipality = matchMunicipalityName(
    question,
    directory.map((item) => item.localidad)
  )
  if (!municipality && !processionDay) return null

  const municipalityKey = normalizeFreeFactText(municipality)
  const brotherhoods = directory
    .filter((item) => !municipality || normalizeFreeFactText(item.localidad) === municipalityKey)
    .filter((item) => !brotherhoodType || (item.tipos || []).some((type) => (
      normalizeFreeFactText(type) === normalizeFreeFactText(brotherhoodType)
    )))
    .filter((item) => !processionDay || normalizeFreeFactText(holyWeekDay(item)) === normalizeFreeFactText(processionDay))
    .sort((a, b) => {
      const firstLocality = localityLabel(a) || a.localidad || ''
      const secondLocality = localityLabel(b) || b.localidad || ''
      const firstCapital = firstLocality === 'Sevilla capital' ? 0 : 1
      const secondCapital = secondLocality === 'Sevilla capital' ? 0 : 1
      if (firstCapital !== secondCapital) return firstCapital - secondCapital
      const localityDifference = firstLocality.localeCompare(secondLocality, 'es', { sensitivity: 'base' })
      if (localityDifference) return localityDifference
      return String(a.nombrePopular || a.nombreOficial).localeCompare(String(b.nombrePopular || b.nombreOficial), 'es')
    })

  if (!brotherhoods.length) {
    if (processionDay && municipality) {
      return notDocumented(`Todavía no hay hermandades publicadas del ${processionDay} en ${municipality}.`)
    }
    if (processionDay) {
      return notDocumented(`Todavía no hay hermandades publicadas del ${processionDay} en Sevilla capital y la provincia.`)
    }
    const collection = brotherhoodCollectionLabel(brotherhoodType, 2).replace(' publicadas', '')
    return notDocumented(`Todavía no hay ${collection} publicadas de ${municipality} en Hilo Cofrade.`)
  }

  const visible = brotherhoods.slice(0, 30)
  const entityIds = visible.map((item) => item.id).filter(Boolean).slice(0, 12)
  const truncated = visible.length < brotherhoods.length
  const parishGrouping = municipality
    ? brotherhoods.find((item) => hasDirectoryType(item, 'agrupaciones-parroquiales'))
    : null
  const sacramental = municipality
    ? brotherhoods.find((item) => hasDirectoryType(item, 'sacramentales'))
    : null
  const dayDirectoryHref = municipality && processionDay ? directoryPath(brotherhoods[0], 'semana-santa') : ''
  const links = []

  if (processionDay) {
    links.push(dayDirectoryHref
      ? { label: `Ver el ${processionDay} de ${municipality}`, href: dayDirectoryHref }
      : { label: 'Ver la Semana Santa por jornadas', href: '/hermandades/semana-santa' }
    )
  } else {
    if (parishGrouping) {
      links.push({
        label: `Ver las agrupaciones parroquiales de ${municipality}`,
        href: `/hermandades/agrupaciones-parroquiales/${localitySlug(parishGrouping)}`,
      })
    }
    if (sacramental) {
      links.push({
        label: `Ver las sacramentales de ${municipality}`,
        href: `/hermandades/sacramentales/${localitySlug(sacramental)}`,
      })
    }
  }

  const answerText = processionDay
    ? municipality
      ? `En Hilo Cofrade hay ${brotherhoods.length} ${brotherhoods.length === 1 ? 'hermandad publicada' : 'hermandades publicadas'} del ${processionDay} en ${municipality}.${truncated ? ` Te muestro las primeras ${visible.length}.` : ''}`
      : `En Hilo Cofrade hay ${brotherhoods.length} ${brotherhoods.length === 1 ? 'hermandad publicada' : 'hermandades publicadas'} del ${processionDay} en Sevilla capital y la provincia, agrupadas por localidad.${truncated ? ` Te muestro las primeras ${visible.length}.` : ''}`
    : `En Hilo Cofrade hay ${brotherhoods.length} ${brotherhoodCollectionLabel(brotherhoodType, brotherhoods.length)} de ${municipality}.${truncated ? ` Te muestro las primeras ${visible.length}.` : ''}`

  const path = processionDay
    ? ['Hermandades', 'Semana Santa', ...(municipality ? [municipality] : ['Sevilla y provincia']), processionDay]
    : ['Hermandades', ...(brotherhoodType ? [brotherhoodType] : []), municipality, 'Directorio publicado']

  const contextLabel = processionDay
    ? municipality
      ? `${brotherhoods.length} ${brotherhoods.length === 1 ? 'hermandad' : 'hermandades'} del ${processionDay} en ${municipality}`
      : `${brotherhoods.length} ${brotherhoods.length === 1 ? 'hermandad' : 'hermandades'} del ${processionDay} en Sevilla y provincia`
    : `${brotherhoods.length} ${brotherhoodCollectionLabel(brotherhoodType, brotherhoods.length).replace(' publicadas', '')} de ${municipality}`

  const evidenceScope = municipality || 'Sevilla y provincia'
  return answer({
    text: answerText,
    path,
    items: visible.map((item) => ({
      label: item.nombrePopular || item.nombreOficial,
      meta: [item.tipos?.join(' · '), item.diaSalida, item.sede].filter(Boolean).join(' · '),
      href: `/hermandades/${item.slug}`,
      group: processionDay
        ? (municipality ? processionDay : (localityLabel(item) || item.localidad || 'Provincia'))
        : brotherhoodGroup(item),
    })),
    links,
    evidence: [{
      key: `brotherhood-directory-${normalizeFreeFactText(evidenceScope).replace(/\s+/g, '-')}-${normalizeFreeFactText(processionDay || brotherhoodType || 'all').replace(/\s+/g, '-')}`,
      label: 'Directorio público de Hermandades',
      detail: `${brotherhoods.length} fichas publicadas · ${evidenceScope}${processionDay ? ` · ${processionDay}` : ''}`,
    }],
    followUps: processionDay
      ? ['¿Qué tienen en común?', '¿Cuáles tienen acompañamiento musical publicado?']
      : ['¿Qué tienen en común?', 'Ordénalas de más antigua a más reciente.'],
    compactItemLimit: brotherhoods.length <= 12 ? brotherhoods.length : 5,
    context: entityIds.length ? {
      entityId: null,
      entityType: null,
      name: municipality || processionDay,
      resultSet: {
        entityType: 'brotherhood',
        entityIds,
        count: entityIds.length,
        label: contextLabel,
        sourceIntent: processionDay
          ? (municipality ? 'brotherhoods_by_day_and_municipality' : 'brotherhoods_by_day')
          : 'brotherhoods_by_municipality',
      },
    } : null,
  })
}

async function municipalityDirectory(question, requestedKind = 'overview') {
  const [brotherhoodDirectory, bandDirectory] = await Promise.all([
    getHermandadesDirectory(),
    getPublicBandsDirectory(),
  ])
  const municipality = matchMunicipalityName(question, [
    ...brotherhoodDirectory.map((item) => item.localidad),
    ...bandDirectory.map((item) => item.municipality),
  ])
  if (!municipality) return null

  const municipalityKey = normalizeFreeFactText(municipality)
  const brotherhoods = brotherhoodDirectory
    .filter((item) => normalizeFreeFactText(item.localidad) === municipalityKey)
    .sort((a, b) => String(a.nombrePopular || a.nombreOficial).localeCompare(String(b.nombrePopular || b.nombreOficial), 'es'))
  const bands = bandDirectory
    .filter((item) => normalizeFreeFactText(item.municipality) === municipalityKey)
    .sort((a, b) => String(a.popularName || a.name).localeCompare(String(b.popularName || b.name), 'es'))

  if (requestedKind === 'bands') {
    if (!bands.length) return notDocumented(`Todavía no hay Bandas publicadas de ${municipality} en Hilo Cofrade.`)
    const visible = bands.slice(0, 30)
    const entityIds = visible.map((item) => item.id).filter(Boolean).slice(0, 12)
    return answer({
      text: `En Hilo Cofrade hay ${bands.length} ${bands.length === 1 ? 'formación musical publicada' : 'formaciones musicales publicadas'} de ${municipality}.`,
      path: ['Bandas', municipality, 'Directorio publicado'],
      items: visible.map((item) => ({
        label: item.popularName || item.name,
        meta: [item.type, item.municipality].filter(Boolean).join(' · '),
        href: `/bandas/${item.slug}`,
      })),
      evidence: [{
        key: `band-directory-${municipalityKey.replace(/\s+/g, '-')}`,
        label: 'Directorio público de Bandas',
        detail: `${bands.length} fichas publicadas en ${municipality}`,
      }],
      followUps: ['¿A qué Hermandades acompaña cada una?', '¿Cuáles son bandas de música?'],
      context: entityIds.length ? {
        entityId: null,
        entityType: null,
        name: municipality,
        resultSet: {
          ...
          entityIds,
          count: entityIds.length,
          label: `${bands.length} bandas de ${municipality}`,
          sourceIntent: 'bands_by_municipality',
        },
      } : null,
    })
  }

  if (!brotherhoods.length && !bands.length) {
    return notDocumented(`Todavía no hay Hermandades ni Bandas publicadas de ${municipality} en Hilo Cofrade.`)
  }

  return answer({
    text: `En ${municipality}, Hilo Cofrade publica ${brotherhoods.length} ${brotherhoods.length === 1 ? 'corporación' : 'corporaciones'} y ${bands.length} ${bands.length === 1 ? 'formación musical' : 'formaciones musicales'}.`,
    path: [municipality, 'Directorio municipal', 'Entidades publicadas'],
    items: [
      ...brotherhoods.map((item) => ({
        label: item.nombrePopular || item.nombreOficial,
        meta: ['Hermandad', item.diaSalida].filter(Boolean).join(' · '),
        href: `/hermandades/${item.slug}`,
      })),
      ...bands.map((item) => ({
        label: item.popularName || item.name,
        meta: ['Banda', item.type].filter(Boolean).join(' · '),
        href: `/bandas/${item.slug}`,
      })),
    ].slice(0, 30),
    evidence: [{
      key: `municipality-directory-${municipalityKey.replace(/\s+/g, '-')}`,
      label: 'Directorios públicos por municipio',
      detail: `${brotherhoods.length} Hermandades y ${bands.length} Bandas publicadas en ${municipality}`,
    }],
    followUps: [`Hermandades de ${municipality}`, `Bandas de ${municipality}`],
    context: null,
  })
}
...
export async function askHiloCofradeFreeFacts(question, context = null) {
  ...
  try {
    if (intent.kind === 'brotherhoods_by_municipality') {
      return await brotherhoodsByMunicipality(question, {
        brotherhoodType: intent.brotherhoodType,
        processionDay: intent.processionDay,
      })
    }
    if (intent.kind === 'bands_by_municipality') return await municipalityDirectory(question, 'bands')
    if (intent.kind === 'municipality_overview') return await municipalityDirectory(question)
