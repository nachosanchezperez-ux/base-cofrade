...
async function brotherhoodsByMunicipality(question) {
  ...
  if (!brotherhoods.length) {
    return notDocumented(`Todavía no hay Hermandades publicadas de ${municipality} en Hilo Cofrade.`)
  }
  ...
  return answer({
    text: `En Hilo Cofrade hay ${brotherhoods.length} ${brotherhoods.length === 1 ? 'corporación publicada' : 'corporaciones publicadas'} de ${municipality}.${truncated ? ` Te muestro las primeras ${visible.length}.` : ''}`,
    path: ['Hermandades', municipality, 'Directorio publicado'],
    ...
    context: entityIds.length ? {
      ...
      resultSet: {
        ...
        label: `${brotherhoods.length} corporaciones de ${municipality}`,
        sourceIntent: 'brotherhoods_by_municipality',
      },
    } : null,
  })
}
...
export async function askHiloCofradeFreeFacts(question, context = null) {
  ...
  try {
    if (intent.kind === 'brotherhoods_by_municipality') return await brotherhoodsByMunicipality(question)
