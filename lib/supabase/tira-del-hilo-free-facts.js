...
async function brotherhoodsByMunicipality(question, { brotherhoodType = '', processionDay = '' } = {}) {
  ...
  if (!brotherhoods.length) {
    if (processionDay) return notDocumented(`Todavía no hay hermandades publicadas del ${processionDay} en ${municipality}.`)
  ...
  const parishGrouping = brotherhoods.find((item) => hasDirectoryType(item, 'agrupaciones-parroquiales'))
  const dayDirectoryHref = processionDay ? directoryPath(brotherhoods[0], 'semana-santa') : ''
  const parishGroupingHref = !processionDay && parishGrouping
    ? `/hermandades/agrupaciones-parroquiales/${localitySlug(parishGrouping)}`
    : ''
  const links = dayDirectoryHref
    ? [{ label: `Ver el ${processionDay} de ${municipality}`, href: dayDirectoryHref }]
    : parishGroupingHref
      ? [{ label: `Ver las agrupaciones parroquiales de ${municipality}`, href: parishGroupingHref }]
      : []
  ...
  const path = processionDay
    ? ['Hermandades', 'Semana Santa', municipality, processionDay]
  ...
  return answer({
    ...
    items: visible.map((item) => ({
      ...
      group: processionDay || brotherhoodGroup(item),
    })),
    links,
    evidence: [{
      key: `brotherhood-directory-${municipalityKey.replace(/\s+/g, '-')}`,
      label: 'Directorio público de Hermandades',
      detail: `${brotherhoods.length} fichas publicadas en ${municipality}`,
    }],
    followUps: ['¿Qué tienen en común?', 'Ordénalas de más antigua a más reciente.'],
    ...
    context: entityIds.length ? {
      ...
      resultSet: {
        ...
        label: contextLabel,
        sourceIntent: processionDay ? 'brotherhoods_by_day_and_municipality' : 'brotherhoods_by_municipality',
      },
    } : null,
  })
}
