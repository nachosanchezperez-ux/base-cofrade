...
export async function getPanelMasterData() {
  ...
  const [advocationEntitiesResult, advocationsResult, municipalitiesResult, placesResult, imagesResult, imageEntitiesResult, sourceLinksResult] = await Promise.all([
  ...
  const imageOptions = imageEntities.map((entity) => ({
    ...entity,
    advocation_entity_id: imageById.get(entity.id)?.advocation_entity_id || null,
  }))
