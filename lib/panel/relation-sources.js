...
const RELATION_SOURCE_CONFIG = {
  ...
  march_author: { mode: 'scope' },
  band_agent: { mode: 'scope' },
  ...
  march_dedication: { mode: 'column', column: 'march_dedication_id' },
  march_recording: { mode: 'column', column: 'march_recording_id' },
}
...
export async function loadRelationSourceSupport(supabase, relations, relationKind) {
  ...
  const [sources, sourceLinks] = await Promise.all([
    fetchAllPages(
      () => supabase
        .from('sources')
      ...
      ? fetchAllPages(
          () => config.mode === 'scope'
            ...
            : supabase
                ...
                .order('created_at')
                .order('id'),
          'No se pudieron cargar las Fuentes de las relaciones',
        )
      : Promise.resolve([]),
  ])
