...
export function validateBulkImportDatabaseChecks(table, payload = {}) {
  const errors = []

  if (
    table === 'music_accompaniment_periods'
    && payload.date_from == null
    && payload.date_from_text == null
    && payload.year_from == null
  ) {
    errors.push('CHECK_CONSTRAINT: music_accompaniment_periods.music_period_start_present requiere date_from, date_from_text o year_from.')
  }
...
export async function preflightBulkImportRecord(supabase, input, context = null) {
  ...
  if (effectiveOperation) {
    ...
    const databasePayload = effectiveOperation === 'update'
      ? { ...(existing || {}), ...payload }
      : payload
    errors.push(...validateBulkImportDatabaseChecks(record.table, databasePayload))
  }
...
export async function applyBulkImportRecord(supabase, input) {
  ...
  let result
  if (preflight.effectiveOperation === 'update') {
    const columns = bulkImportConflictColumns(record)
    let query = supabase.from(record.table).update(payload)
    for (const column of columns) query = query.eq(column, payload[column])
    result = await query
  } else {
    const options = record.on_conflict ? { onConflict: record.on_conflict } : undefined
    result = record.operation === 'upsert'
