...
export async function getBrotherhoodEditorData(id) {
  ...
  const [brotherhood, colors, socialLinks, series, cults, heritage, heritageAssets, municipalities, places] = await Promise.all([
...
export async function getBandEditorData(id) {
  ...
  const [bandResult, namesResult, directionResult, periodsResult, premieresResult, assignmentsResult, municipalitiesResult, brotherhoodEntitiesResult, linkedBrotherhoodRelationResult] = await Promise.all([
  ...
  return {
    ...
    premieres: premieres.map((item) => ({ ...item, source: sourceById.get(item.source_id) || null })),
