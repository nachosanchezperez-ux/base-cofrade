...
export async function getPanelDashboard() {
  ...
  const [published, review, draft, archived, media, outings, cults, crewEvents, heritageAssets, heritageUpdates, activity] = await Promise.all([
    ...
    supabase.from('events').select('*', { count: 'exact', head: true }).eq('event_category', 'crew_call'),
  ...
  return {
    ...
    modules: {
      ...
      crewEvents: count(crewEvents, 'No se pudieron contar las igualás y ensayos'),
