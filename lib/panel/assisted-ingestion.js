...
const ENTITY_SUBTYPE_SPECS = Object.freeze({
  ...
  heritage_asset: {
    ...
    fields: ['asset_type', 'description', 'notes', 'date_from_text', 'date_to_text', 'origin_notes', 'technique', 'materials', 'dimensions_text', 'iconography', 'historical_context', 'provenance_text', 'blessing_date_text', 'usage_text'],
  },
})
...
Relaciones permitidas: has_titular (Hermandad→advocation), titular (Hermandad→image), processional_step (Hermandad→step), processes_on (image→step), belongs_to_brotherhood (band→Hermandad), authored_by (march→agent), image_authored_by (image→agent, autoría documentada), image_attributed_to (image→agent, atribución no documental vigente), dedicated_to (march→entidad).
Para image_attributed_to, propón la relación solo cuando la Fuente mantenga esa atribución como vigente o aceptada. Si la Fuente cuenta una atribución histórica después descartada, corregida o superada por una autoría documentada, consérvala únicamente en warnings/evidence y NO generes la relación.
...
export function shortNamePotentialScore(left, right) {
  const leftTokens = semanticTokens(left)
  const rightTokens = semanticTokens(right)
  ...
  if (shorter.length > 3 || longer.length > 5) return 0

  const prefix = shorter.every((token, index) => longer[index] === token)
  return prefix ? 0.79 : 0
}

function proposalHasStageableRelation(analysis, localId) {
  return (analysis.relations || []).some((relation) => relation.stageable
    && (relation.source_ref === localId || relation.target_ref === localId))
}

function shouldDefaultCreateNew(analysis, entity) {
  if (entity.entity_type === 'heritage_asset') return true
  return proposalHasStageableRelation(analysis, entity.local_id)
}
  ...
  const preliminary = analysis.entities.map((entity) => {
    ...
    for (const variant of variants) {
      ...
      if (entity.entity_type === 'agent') {
        const potentialScore = shortNamePotentialScore(entity.name, variant.matched_name)
        if (!potentialScore) continue
        const previous = potentialById.get(variant.id)
        if (!previous || potentialScore > previous.match_score) {
          potentialById.set(variant.id, {
            ...variant,
            match_score: potentialScore,
            context_match: false,
            match_kind: 'potential',
          })
        }
      }
    }
    ...
    const potential = semantic.length
      ? []
      : [...potentialById.values()].sort((left, right) => right.match_score - left.match_score).slice(0, 8)
        ...
        : semantic.length === 1
          ...
          : semantic.length > 1
            ...
            : potential.length
              ? 'potential'
              : 'new',
      candidates: semantic.length ? semantic : potential,
    }
  })
      ...
      return {
        ...
        resolution: {
          ...
          default_choice: state === 'exact' && enrichedCandidates.length === 1
            ...
            : autoSemantic
              ? `existing:${enrichedCandidates[0].id}`
              : state === 'new' && shouldDefaultCreateNew(analysis, entity)
                ? 'new'
                : 'ignore',
        },
      }
