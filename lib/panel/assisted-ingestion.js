  ...
  if (subtype) {
    for (const [column, proposed] of Object.entries(subtype.data)) {
      ...
      fields.push({
        ...
        state: current == null ? 'fill' : valuesEquivalent(current, proposed) ? 'same' : 'conflict',
      })
    }
  }
...
export async function attachResolutionSuggestions(supabase, analysis, options = {}) {
  ...
  const preliminary = analysis.entities.map((entity) => {
    ...
    const scopedExact = scoped?.size ? exact.filter((candidate) => scoped.has(candidate.id)) : []
    const exactCandidates = scopedExact.length ? scopedExact : exact

    if (exactCandidates.length) {
      return {
        entity,
        state: exactCandidates.length === 1 ? 'exact' : 'ambiguous',
    ...
    for (const variant of variants) {
      ...
      if (scoped?.size && ['image', 'step', 'heritage_asset', 'advocation'].includes(entity.entity_type) && !scoped.has(variant.id)) continue
    ...
    const semantic = [...semanticById.values()]
    ...
    return {
      entity,
      state: semantic.length === 1 ? 'semantic' : semantic.length > 1 ? 'ambiguous' : 'new',
      candidates: semantic,
    }
  })
