...
export async function attachResolutionSuggestions(supabase, analysis) {
  ...
  function addCandidate(entity, matchedName, matchedBy) {
    ...
    const group = index.get(key) || new Map()
    const previous = group.get(entity.id)
    if (!previous || matchedBy === 'canonical') {
      group.set(entity.id, {
        ...entity,
  ...
  for (const entity of entities) addCandidate(entity, entity.name, 'canonical')
  ...
  for (const spec of aliasSpecs) {
    ...
    for (const row of aliasResult.data || []) {
      ...
      for (const column of spec.nameColumns) {
        ...
        if (alias) addCandidate(entity, alias, `${spec.table}.${column}`)
      }
    }
  }
  ...
  return {
    ...
    entities: analysis.entities.map((entity) => {
      const candidates = [...(index.get(`${entity.entity_type}:${normalizeComparableName(entity.name)}`) || new Map()).values()]
        .slice(0, 8)
      return {
        ...entity,
        resolution: {
