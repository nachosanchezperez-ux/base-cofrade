...
const CONSTRAINED_VALUES = new Map([
  ['image_authorships.authorship_type', new Set([
    'author', 'attributed_to', 'workshop_of', 'circle_of', 'school_of', 'anonymous',
  ])],
  ['images.current_condition', new Set(['extant', 'lost', 'destroyed', 'unknown'])],
  ['steps.current_condition', new Set(['preserved', 'partially_preserved', 'not_preserved'])],
  ['events.event_category', new Set(['historical', 'crew_call'])],
])
...
export function validateBulkImportRecord(input) {
  ...
  for (const [field, allowedValues] of CONSTRAINED_VALUES) {
    const [table, column] = field.split('.')
    if (record.table !== table || !hasOwn(record.data, column) || record.data[column] === null) continue
    if (!allowedValues.has(record.data[column])) {
      errors.push(`INVALID_VALUE: ${field} no admite «${String(record.data[column])}». Valores permitidos: ${[...allowedValues].join(', ')}.`)
    }
  }
