...
export async function getPanelAgents({ query = '', status = '', kind = '' } = {}) {
  ...
  let agentQuery = supabase
    .from('agents')
    .select('entity_id, agent_kind, municipality_id')
...
export async function getAgentEditorData(id) {
  ...
  const [agentResult, namesResult, disciplinesResult, rolesResult, municipalitiesResult, mediaResult, sourceLinksResult] = await Promise.all([
    supabase
      .from('agents')
      .select('entity_id, agent_kind, municipality_id, foundation_or_birth_text, death_or_end_text, website_url, instagram_url, description, birth_or_foundation_date, death_or_end_date, address, email, phone, active_notes')
      .eq('entity_id', id)
      .maybeSingle(),
    supabase
      .from('agent_names')
      .select('id, agent_entity_id, name, name_type, date_from, date_from_text, date_to, date_to_text, is_current, notes, created_at')
      .eq('agent_entity_id', id)
      .order('is_current', { ascending: false })
      .order('created_at'),
    supabase
      .from('agent_disciplines')
      .select('id, agent_entity_id, discipline, is_primary, notes')
      .eq('agent_entity_id', id)
      .order('is_primary', { ascending: false })
      .order('discipline'),
    supabase
      .from('agent_roles')
      .select('id, agent_entity_id, role_name, date_from, date_from_text, date_to, date_to_text, notes')
      .eq('agent_entity_id', id)
  ...
  const agent = assertQuery(agentResult, 'No se pudo cargar la ficha del Agente')
  ...
  const directSourceLinks = rows(sourceLinksResult, 'No se pudieron cargar las Fuentes del Agente')
  ...
  const sources = sourceIds.length
    ? rows(
        await supabase
          ...
          .in('id', sourceIds)
          .order('name'),
        'No se pudieron cargar las Fuentes del Agente'
      )
    : []
