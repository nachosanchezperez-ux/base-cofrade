export const DEFAULT_IMPORT_CHUNK_ITEMS = 75
export const DEFAULT_IMPORT_CHUNK_BYTES = 600_000
...
export function parseCsvRows(text, delimiter = detectCsvDelimiter(text)) {
  const rows = []
  let row = []
  let cell = ''
  let quoted = false
...
export function splitImportPayload(records, options = {}) {
  const maxItems = options.maxItems ?? DEFAULT_IMPORT_CHUNK_ITEMS
  const maxBytes = options.maxBytes ?? DEFAULT_IMPORT_CHUNK_BYTES
  if (!Number.isInteger(maxItems) || maxItems < 1) throw new Error('El máximo de registros por bloque debe ser un entero positivo.')
  if (!Number.isInteger(maxBytes) || maxBytes < 1) throw new Error('El máximo de bytes por bloque debe ser un entero positivo.')
  ...
  for (const record of records) {
    ...
    if (recordBytes > maxBytes) throw new Error(`Hay un registro de ${recordBytes} bytes que supera el máximo seguro de ${maxBytes} bytes por envío.`)
