...
export async function getPanelEvents({ query = '', status = '', contextEntityId = '' } = {}) {
  ...
  const historicalRows = rows(
    await supabase.from('events').select('entity_id').eq('event_category', 'historical'),
    'No se pudo separar el archivo histórico de la agenda de cuadrillas'
  )
  let eventIds = historicalRows.map((item) => item.entity_id)
  ...
  if (contextEntityId) {
    ...
    const linkedIds = new Set(links.map((item) => item.source_entity_id).filter(Boolean))
    eventIds = eventIds.filter((id) => linkedIds.has(id))
    if (!eventIds.length) return []
  }

  let entityQuery = supabase.from('entities').select('id, name, slug, summary, status, updated_at').eq('entity_type', 'event').in('id', eventIds).order('name')
  ...
  const eventRows = rows(
    await supabase.from('events').select('entity_id, event_type, event_date, event_date_text, place_id, description').eq('event_category', 'historical').in('entity_id', entities.map((item) => item.id)),
...
export async function getEventEditorData(eventId) {
  ...
  const [entityResult, eventResult, relationsResult, placesResult, targetsResult, mediaResult, directSourcesResult] = await Promise.all([
    ...
    supabase.from('events').select('*').eq('entity_id', eventId).eq('event_category', 'historical').maybeSingle(),
