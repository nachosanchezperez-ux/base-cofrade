...
export async function getPanelDataHealth() {
  ...
  for (const entity of entities.filter((item) => SPECIALIZED_TYPES.includes(item.entity_type))) {
    if (!subtypeIds[entity.entity_type]?.has(entity.id)) {
      issues.push(issue(entity, {
        ...
        title: 'Nodo de referencia sin ficha editable',
        detail: 'Existe en el grafo, pero todavía no tiene su registro especializado. Debe consolidarse antes de completar sus datos.',
