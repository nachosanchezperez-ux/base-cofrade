...
export async function getPanelDataHealth() {
  ...
  const imageWithBrotherhoodIds = new Set(brotherhoodImages.map((item) => item.image_entity_id))
  ...
  for (const entity of published.filter((item) => item.entity_type === 'image')) {
    ...
    if (!imageWithBrotherhoodIds.has(entity.id)) {
      issues.push(issue(entity, {
        key: 'image-brotherhood', severity: 'warning', category: 'Relaciones',
        title: 'Imagen sin Hermandad vigente',
        detail: 'La Imagen está publicada, pero no tiene una relación Hermandad ↔ Imagen publicada y vigente. Si es deliberado, conviene revisar su contexto de pertenencia.',
