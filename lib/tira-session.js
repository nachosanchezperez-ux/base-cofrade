...
function safeInternalHref(value = '') {
  ...
  const entityPage = /^\/(hermandades|imagenes|pasos|bandas|marchas)(?:\/[a-z0-9][a-z0-9-]*){1,4}(?:[?#][a-z0-9=&_%.-]*)?$/i
  const contentPage = /^\/(crucetas-musicales|extraordinarias|procesiones-de-gloria)(?:\/[a-z0-9][a-z0-9-]*)?(?:[?#][a-z0-9=&_%.-]*)?$/i
  const agendaPage = /^\/agenda-cofrade(?:[?#][a-z0-9=&_%.-]*)?$/i
  return entityPage.test(href) || contentPage.test(href) || agendaPage.test(href) ? href : ''
}
...
function sanitizeLink(link) {
  ...
  const label = text(link.label, 220)
...
function sanitizeResponse(response) {
  ...
  return {
    ...
    links: Array.isArray(response.links) ? response.links.map(sanitizeLink).filter(Boolean).slice(0, MAX_LINKS) : [],
    compactItemLimit: Math.max(1, Math.min(Number(response.compactItemLimit) || 3, 12)),
