...
import { absoluteUrl, brotherhoodPublicTypeLabel } from '@/lib/seo';
...
import { resolveSocialImageSource } from '@/lib/social-image-media';
...
export async function renderBrotherhoodSocialImage(slug) {
  ...
  const photoSrc = await resolveSocialImageSource(
    coverMedia?.path ? absoluteUrl(coverMedia.path) : ''
  );
  const title = brotherhood?.nombrePopular || 'Hilo Cofrade';
  const locality = brotherhood?.localidad || 'Sevilla y provincia';
  const typeLabel = brotherhood ? brotherhoodPublicTypeLabel(brotherhood) : 'Enciclopedia cofrade';
