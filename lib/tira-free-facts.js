...
export function freeFactIntent(question = '', contextEntityType = '') {
  ...
  const asksForBandCollection = /\b(cuales|que)\b.*\b(bandas|formaciones musicales)\b.*\b(son|hay|tiene|de|en)\b/.test(q)
    ...
    || /^(bandas|formaciones musicales)\s+(de|en)\b/.test(q)
  ...
  const brotherhoodCollection = /\b(hermandades|cofradias|corporaciones cofrades|agrupaciones parroquiales)\b/.test(q)
  const asksForCollection = /\b(cuales|que)\b.*\b(hermandades|cofradias|corporaciones cofrades|agrupaciones parroquiales)\b.*\b(son|hay|tiene|de|en)\b/.test(q)
    || /\b(lista|listado|muestra|muestrame|dime)\b.*\b(hermandades|cofradias|corporaciones cofrades|agrupaciones parroquiales)\b/.test(q)
    || /^(hermandades|cofradias|corporaciones cofrades|agrupaciones parroquiales)\s+(de|en)\b/.test(q)
  const asksForAnotherRelation = /\b(bandas?|musica|acompanamientos?|pasos?|imagenes?|titulares?)\b/.test(q)
  if (brotherhoodCollection && asksForCollection && !asksForAnotherRelation) {
    return { kind: 'brotherhoods_by_municipality', entityTypes: ['brotherhood'] }
  }

  if (/^(que hay|que puedo encontrar|que tiene)\s+(en|de)\b/.test(q)) {
...
export function matchMunicipalityName(question = '', municipalityNames = []) {
  ...
  return [...new Set(municipalityNames.map((name) => String(name || '').trim()).filter(Boolean))]
    .map((name) => ({ name, normalized: normalizeFreeFactText(name) }))
    .filter((item) => item.normalized.length >= 3 && q.includes(` ${item.normalized} `))
    .sort((a, b) => b.normalized.length - a.normalized.length || a.name.localeCompare(b.name, 'es'))[0]?.name || ''
}
