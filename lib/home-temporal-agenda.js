...
export function buildHomeTemporalAgenda({ items = [], today = '', now = new Date() } = {}) {
  ...
  const liveItems = normalized
    .filter((item) => item.liveState?.isLive)
    .map((item) => withAgendaTemporalDay(item, todayKey))
    .sort(compareItems)
  const todayItems = normalized
    .filter((item) => overlapsDay(item, todayKey))
    .map((item) => withAgendaTemporalDay(item, todayKey))
    .sort(compareItems)
  const remainingTodayItems = todayItems
    .filter((item) => isRemainingToday(item, todayKey, clock.minutes))
    .sort(compareItems)
  const tomorrowItems = normalized
    .filter((item) => overlapsDay(item, tomorrow))
    .map((item) => withAgendaTemporalDay(item, tomorrow))
    .sort(compareItems)
  const weekendItems = normalized
    .filter((item) => overlapsRange(item, weekendStart, weekendEnd))
    .map((item) => withAgendaTemporalDay(
      item,
      agendaTemporalRangeDate(item, weekendStart, weekendEnd) || item.date
    ))
    .sort(compareItems)
