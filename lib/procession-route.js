...
function normalizePoint(point, index) {
  ...
  return {
    ...
    annotations: Array.isArray(point?.annotations) ? point.annotations : [],
  }
}
...
function pointAnnotation(point, schedule = [], music = []) {
  ...
  music.forEach((item) => {
    const labels = [item?.start, item?.end].filter(Boolean)
    if (!labels.some((label) => placeMatches(point.label, label))) return
    annotations.push({
      type: 'music',
...
function prepareLegs({ legs, origin, destination, circuit, schedule, music }) {
  ...
  return legs.map((leg, index) => {
    ...
    const finalPoints = points.map((point, pointIndex) => {
      ...
      return {
        ...
        role,
        annotations: pointAnnotation(point, schedule, music),
      }
    })
