...
const AGENDA_TOPIC = /\b(agenda|acto|actos|evento|eventos|procesion|procesiones|rosario|rosarios|besamanos|besapies|concierto|conciertos|traslado|traslados)\b/
const AGENDA_TIME = /\b(hoy|manana|fin de semana|finde|esta semana)\b/
...
export function publishedContentIntent(question = '') {
  ...
  if (AGENDA_TOPIC.test(q)
    || (/^(?:que hay|que puedo ver|que puedo encontrar|que hacer)\b/.test(q) && AGENDA_TIME.test(q))) {
    return { kind: 'agenda' }
  }
  return null
}
