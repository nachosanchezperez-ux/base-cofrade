...
export function buildMunicipalityAgendaHub({
  ...
  const localImages = images.filter((item) => matches(item.municipality || item.municipalitySlug))
  const localSteps = steps.filter((item) => matches(item.municipality || item.municipalitySlug))
  ...
  return {
    ...
    imageDirectorySlug: localDirectorySlug(localImages, defaultHeritageSlug),
    stepDirectorySlug: localDirectorySlug(localSteps, defaultHeritageSlug),
    exists: Boolean(
      items.length
      || localBrotherhoods.length
      || localBands.length
      || localImages.length
      || localSteps.length
    ),
