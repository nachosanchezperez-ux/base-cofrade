...
export function refersToPreviousSet(question = '') {
  ...
  return /\b(cada uno|cada una|cada cual|cuales|ellos|ellas|estos|estas|esos|esas|los anteriores|las anteriores|los resultados|las encontradas|los encontrados|todas|todos|entre ellas|entre ellos|de ellas|de ellos|de esas|de esos|solo|solamente)\b/.test(q)
    || /^y\b/.test(q)
...
export function genericSetIntent(question = '', entityType = '') {
  ...
  const compare = /\b(compara|comparar|comparacion|diferencias|diferencian|comparamelos|comparamelas)\b/.test(q)
  ...
  if (entityType === 'band') {
    ...
    if (refers && /\b(mas|mayor)\b/.test(q) && /\bhermandad|hermandades\b/.test(q)) return 'band_most_brotherhoods'
  ...
  if (entityType === 'step') {
    if (refers && /\b(quien|capataz|capataz|lleva|dirige|manda|personal)\b/.test(q)) return 'step_personnel'
