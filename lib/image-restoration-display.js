...
export function buildImageChronology(image = {}, cards = imageRestorationCards(image.restauraciones)) {
  ...
  for (const card of cards) {
    ...
    // Preserve the existing chronology, including textual dating such as centuries.
    // Insert before a later explicit year; never fabricate dates for undated entries.
    const year = Number(yearsIn(entry.fecha)[0]);
    const position = year ? entries.findIndex((item) => {
      const otherYear = Number(yearsIn(item.fecha)[0]);
      return otherYear && otherYear > year;
    }) : -1;
    if (position < 0) entries.push(entry);
    else entries.splice(position, 0, entry);
