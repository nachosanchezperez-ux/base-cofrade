...
--
-- Una sección solo deja de usar el fallback local cuando el Panel ha tomado
-- control explícito sobre ella. De esta forma, archivar o vaciar una sección
-- no hace reaparecer contenido histórico embebido en código.
...
declare
  ...
  relation_row record;
begin
  -- Acontecimientos requieren resolver la Hermandad desde sus relaciones.
  ...
  -- Primero intenta usar el contexto directo del audit_log.
  ...
  -- Relaciones musicales suelen auditar la Banda o el Paso como entidad raíz.
  ...
  -- No reclamar una sección por el mero hecho de crear un borrador.
  ...
  -- Los colores se guardan junto a la identidad en el editor general.
  if new.object_type = 'brotherhood' then
