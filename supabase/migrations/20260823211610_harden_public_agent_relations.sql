...
using (
  status = 'published'
  and exists (
    select 1 from public.entities image
...
using (
  status = 'published'
  and exists (
    select 1 from public.entities march
...
using (
  ...
  and (
    ...
    or exists (
      select 1 from public.entities element
...
using (
  status = 'published'
  and exists (
    select 1 from public.entities target
...
using (
  exists (
    select 1 from public.heritage_updates heritage_update
    join public.entities brotherhood on brotherhood.id = heritage_update.brotherhood_entity_id
...
using (
  status = 'published'
  and exists (
    select 1 from public.entities source
