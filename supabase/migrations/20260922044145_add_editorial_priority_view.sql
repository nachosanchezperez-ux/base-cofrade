...
signals as (
  select
    ...
    greatest(0, floor(extract(epoch from (now() - t.updated_at)) / 86400))::int as update_age_days,
    case
...
scored as (
  select
    ...
    (
      ...
      + case
        when s.updated_at >= now() - interval '14 days' then 20
        when s.updated_at >= now() - interval '45 days' then 12
        when s.updated_at >= now() - interval '120 days' then 5
        else 0
      end
      + case
