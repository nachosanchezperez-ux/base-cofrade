...
- La cronología común permite consultar hoy, mañana, el fin de semana y próximos actos, y filtrar por procesiones, traslados, rosarios, besamanos y besapiés, además de por territorio.
- Las intenciones temporales principales tienen URLs canónicas estables: `/agenda-cofrade/hoy`, `/agenda-cofrade/manana` y `/agenda-cofrade/fin-de-semana`. Cada landing resume Sevilla capital/provincia, tipos de acto y municipios con actividad, y mantiene los filtros con query para combinaciones más específicas.
- En estas landings la UX prioriza consulta rápida: hero compacto con resumen, selector temporal sticky, filtros de territorio/tipo antes de la cronología y una lista por jornada con hora en primera columna; en móvil categorías y municipios pasan a rails horizontales y la cronología se compacta sin perder localidad ni acciones.
- La micro-UX móvil reduce la altura del selector temporal, mantiene targets táctiles de al menos 40–44 px, marca la localidad como chip y añade una señal discreta de deslizamiento en los rails horizontales.
- Procesiones, traslados, rosarios, besamanos/besapiés y conciertos comparten un código visual contenido: cada tipo usa un acento y un fondo suave repetidos en filtro y cronología, sin cambiar la estructura ni convertir la Agenda en una interfaz multicolor.
