...
**Producción vigente tras #858:** READY · deployment `dpl_CqbFFDxWiwadWZiyyyv7Z4JTGjnt` · SHA `2c9d30b36ad4f4f2a9c4bb828de5be069fc3bce4` · `hilocofrade.es` sin error de alias

**Supabase:** `ACTIVE_HEALTHY` · 12/12 migraciones estructurales activas · Estepa `c0160032-0000-4000-8000-000000000001` cerrada con **495/495 aplicadas · 0 inválidas · 0 fallos**
...
**Frente editorial ACTIVO:** **Lebrija · sexto macrolote municipal HC-016 · MANIFIESTO DETERMINISTA**. IDs `c0160033-*`, slugs y posiciones **465/465** congelados: 463 upsert + 2 update + 0 delete, 32 REUSE. El bulk import `c0160033-0000-4000-8000-000000000001` está solo reservado documentalmente. **0 staging · 0 Apply**.
...
| **ACTIVO · MANIFIESTO DETERMINISTA** | Sexto macrolote municipal HC-016 · Lebrija | **465/465** posiciones congeladas con namespace `c0160033-*`, 463 upsert + 2 update + 0 delete, 32 REUSE; import reservado documentalmente sin fila en Supabase | Siguiente puerta: **payload SQL exacto → BEGIN/ROLLBACK → preflight global → 0 residuos**. **0 staging / 0 Apply** |
...
**Único frente editorial municipal activo: Lebrija · sexto macrolote HC-016 · MANIFIESTO DETERMINISTA.**

El recálculo provincial y el preflight específico siguen cerrados. El manifiesto determinista congela **465/465 posiciones**, namespace `c0160033-*`, 463 upsert + 2 update, 0 delete y 32 REUSE. El import `c0160033-0000-4000-8000-000000000001` continúa sin materializar.

Siguiente movimiento: **construir payload SQL exacto de las 465 DML → validar contrato BEGIN/ROLLBACK → ejecutarlo completo con ROLLBACK → certificar 0 residuos y 0 colisiones**.
