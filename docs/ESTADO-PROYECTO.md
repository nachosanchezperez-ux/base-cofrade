...
**Frente editorial ACTIVO:** **Lebrija · sexto macrolote municipal HC-016 · ROW-BY-ROW CERRADO**. Preflight específico resuelto: **473 DML = 471 upsert + 2 update + 0 delete**, **32 REUSE**, 24 Imágenes canónicas, 23 Pasos, 14 Salidas nuevas, 16 series y 123 source_links. **0 staging · 0 Apply**.
...
| **ACTIVO · ROW-BY-ROW CERRADO** | Sexto macrolote municipal HC-016 · Lebrija | Preflight específico en verde: **473 DML**, 471 upsert + 2 update, 0 delete, 32 REUSE; inventario físico y Fuentes reconciliados; namespace `c0160033-*` todavía sin materializar | Siguiente puerta: **IDs deterministas → manifiesto 473/473 → SQL completo con ROLLBACK**. **0 staging / 0 Apply** |
...
Siguiente movimiento, sin necesidad de una nueva selección: **revalidar colisiones → congelar IDs/slugs `c0160033-*` → manifiesto determinista 473/473 → generar payload SQL exacto → ejecutar el payload completo dentro de transacción + ROLLBACK**.
