# Auditoría del octavo macrolote municipal HC-016 · selección de Carmona

**Corte:** 20 de septiembre de 2026  
**Ámbito:** provincia de Sevilla, excluida Sevilla capital  
**Resultado:** Carmona queda seleccionada como candidata única para el octavo macrolote municipal  
**Límite de esta puerta:** selección documental; no abre inventario, staging, SQL ni Apply

## 1. Regla temporal de 2026

La Semana Santa de 2026 ya es histórica. Sus estaciones de penitencia, horarios y recorridos se tratarán como hechos pasados:

- un recorrido de 2026 no puede presentarse como agenda futura;
- una salida solo podrá quedar en `held` cuando exista evidencia posterior o una fuente que acredite que se celebró;
- si la celebración no puede confirmarse, se conservará la incertidumbre y no se convertirá el anuncio en hecho por el mero paso del tiempo;
- los itinerarios seguirán siendo históricos y reutilizables en la ficha de la Salida, no convocatorias activas.

## 2. Exclusiones

El recálculo parte de cero y excluye los **nueve municipios ya cerrados en siete macrolotes**:

1. Gerena;
2. Dos Hermanas;
3. Alcalá de Guadaíra;
4. Pilas;
5. Cantillana;
6. Coria del Río;
7. Estepa;
8. Lebrija;
9. Osuna.

Sevilla capital también queda fuera del frente municipal. La formulación previa «siete municipios» era imprecisa: son siete macrolotes municipales y nueve municipios cerrados.

## 3. Método

Se combinaron cinco dimensiones:

| Dimensión | Peso | Pregunta |
|---|---:|---|
| Deuda acreditada | 40 | ¿Cuánto universo municipal verificable falta en Hilo Cofrade? |
| Fuente actual | 20 | ¿Existe programa o inventario institucional de 2026? |
| Rendimiento relacional | 15 | ¿Cuántas Hermandades, Imágenes, Pasos, Salidas, Bandas y Fuentes puede aportar? |
| Cerrabilidad | 15 | ¿Puede definirse y cerrarse el universo sin perseguir exhaustividad artificial? |
| Riesgo inverso | 10 | ¿Qué coste tienen duplicados, homónimos y conciliación con nodos existentes? |

La puntuación es comparativa y sirve para ordenar la puerta operativa; no sustituye el inventario canónico posterior.

## 4. Fotografía viva de Supabase

Lectura directa, sin escrituras, sobre el proyecto productivo `kcevwkucqzcyrqaimyhl`:

| Municipio | Hermandades | Bandas | Imágenes | Pasos | Salidas 2026 | Enlaces de Fuente núcleo | Señal principal |
|---|---:|---:|---:|---:|---:|---:|---|
| Carmona | 0 | 0 | 0 | 0 | 1 | 1 | Vacío estructural; la única Salida es letífica y no cubre la Semana Santa |
| Écija | 1 | 1 | 1 | 1 | 1 | 6 | El único nodo es la Hermandad de la Virgen del Valle, ajena al universo penitencial |
| Utrera | 3 | 4 | 3 | 3 | 4 | 16 | Base parcial; dos nodos Álvarez Quintero requieren desambiguación |
| Sanlúcar la Mayor | 1 | 2 | 1 | 0 | 2 | 6 | Solo Vera Cruz representa hoy el universo cofrade |
| Marchena | 1 | 1 | 0 | 0 | 1 | 3 | El nodo existente es la Divina Pastora, no el conjunto penitencial |
| La Rinconada | 5 | 1 | 11 | 10 | 9 | 21 | Cobertura bastante más madura; menor deuda marginal |

## 5. Evidencia externa actual

- La ficha institucional de Turismo de la Provincia de Sevilla para [Carmona 2026](https://www.turismosevilla.org/es/eventos-y-fiestas/semana-santa-2026-carmona) usa la expresión **«hasta nueve hermandades»**. El inventario posterior la normaliza como ocho hermandades penitenciales del Consejo más la Orden Seglar Servita: nueve sujetos corporativos y diez Salidas de 2026, porque la Esperanza aporta dos cortejos. En Supabase no hay ninguna Hermandad de Carmona.
- Turismo provincial mantiene también programas específicos de [Écija 2026](https://www.turismosevilla.org/es/eventos-y-fiestas/semana-santa-2026-ecija), [Utrera 2026](https://www.turismosevilla.org/es/eventos-y-fiestas/semana-santa-2026-utrera) y [Marchena 2026](https://www.turismosevilla.org/es/eventos-y-fiestas/semana-santa-2026-marchena).
- El Ayuntamiento de Sanlúcar la Mayor publicó en 2026 sus recorridos, horarios y cultos; como apoyo de contraste, la [Agenda Cultural de Andalucía](https://www.juntadeandalucia.es/cultura/agendaculturaldeandalucia/evento/semana-santa-de-sanlucar-la-mayor) enumera el núcleo procesional tradicional.

Las páginas institucionales de 2026 prueban actualidad y permiten abrir un inventario verificable. La identidad, denominación canónica y celebración efectiva de cada Salida deberán confirmarse fila a fila en la siguiente puerta.

## 6. TOP 5 nuevo

| Posición | Municipio | Puntuación | Lectura operativa |
|---:|---|---:|---|
| 1 | **Carmona** | **92/100** | Nueve sujetos corporativos acreditados, cero cargados, diez Salidas de 2026 y alcance medio cerrable |
| 2 | **Écija** | **88/100** | Deuda absoluta muy alta y gran rendimiento, con mayor complejidad patrimonial y nominal |
| 3 | **Utrera** | **83/100** | Deuda alta y fuentes activas, pero conciliación más delicada con tres Hermandades y cuatro Bandas existentes |
| 4 | **Sanlúcar la Mayor** | **79/100** | Universo manejable y muy incompleto; menor impacto que el TOP 3 |
| 5 | **Marchena** | **75/100** | Vacío penitencial relevante y fuente 2026; rendimiento esperado algo menor |

## 7. TOP 3 y decisión

1. **Carmona**
2. **Écija**
3. **Utrera**

### Por qué gana Carmona

Carmona ofrece el mejor equilibrio entre impacto y riesgo:

- deuda completa y visible: **0 de 9** sujetos corporativos del programa de Semana Santa, formados por ocho hermandades penitenciales y la Orden Seglar Servita;
- fuente institucional específica de 2026;
- tamaño suficiente para un macrolote de alto rendimiento, pero más controlable que Écija o Utrera;
- superficie de duplicados baja: no hay Hermandades ni Bandas municipales que conciliar;
- la única Salida actual de Carmona es una procesión letífica de septiembre y no colisiona con el universo penitencial.

Écija queda segunda porque su impacto potencial es muy alto, pero exige una investigación patrimonial y nominal más extensa. Utrera queda tercera porque dispone de una base parcial útil, aunque la coexistencia de nodos y las dos denominaciones Álvarez Quintero elevan el riesgo de duplicación.

## 8. Decisión y siguiente puerta

**Carmona queda seleccionada como octavo macrolote municipal HC-016.**

En el momento de su cierre, esta auditoría no autorizaba inventario, staging, SQL, Apply ni una PR editorial. La orden posterior abrió únicamente:

1. inventario canónico de los nueve sujetos corporativos;
2. matriz de Fuentes por entidad y relación;
3. conciliación de la Salida letífica ya existente;
4. clasificación histórica de las Salidas de Semana Santa 2026;
5. detección de homónimos, sedes, Imágenes, Pasos, Bandas y duplicados antes de preparar operaciones.

La puerta de inventario y matriz quedó cerrada en [`INVENTARIO-CARMONA-OCTAVO-MACROLOTE-MUNICIPAL-HC016-2026-09-20.md`](./INVENTARIO-CARMONA-OCTAVO-MACROLOTE-MUNICIPAL-HC016-2026-09-20.md) y [`MATRIZ-FUENTES-CARMONA-OCTAVO-MACROLOTE-MUNICIPAL-HC016-2026-09-20.md`](./MATRIZ-FUENTES-CARMONA-OCTAVO-MACROLOTE-MUNICIPAL-HC016-2026-09-20.md). La corrección no altera la selección de Carmona ni su puntuación: precisa la ontología y eleva el universo procesional de 2026 a diez Salidas históricas.
