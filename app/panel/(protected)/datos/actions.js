...
function slugify(input) {
  return String(input || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')
}
function optionalNumber(formData, name) {
  const candidate = value(formData, name)
  if (!candidate) return null
  const parsed = Number.parseFloat(candidate.replace(',', '.'))
  if (!Number.isFinite(parsed)) throw new Error(`${name} debe ser un número válido.`)
  return parsed
}
function optionalDate(formData, name) {
  const candidate = value(formData, name)
  if (!candidate) return null
  if (!/^\d{4}-\d{2}-\d{2}$/.test(candidate) || Number.isNaN(Date.parse(`${candidate}T00:00:00Z`))) throw new Error(`La fecha de ${name} no es válida.`)
  return candidate
}
...
async function audit(supabase, user, entry) {
  ...
  if (error) console.error('[Hilo Cofrade] No se pudo registrar la edición de datos maestros', error)
}
async function ensureEntitySlug(supabase, slug, exceptId = null) {
  let query = supabase.from('entities').select('id, name').eq('slug', slug).limit(1)
  if (exceptId) query = query.neq('id', exceptId)
  const result = await query
  if (result.error) throw new Error(`No se pudo comprobar el slug: ${result.error.message}`)
  if (result.data?.length) throw new Error(`El slug «${slug}» ya pertenece a ${result.data[0].name}.`)
}
...
export async function saveAdvocationAction(formData) {
  ...
  const id = optionalUuid(formData, 'advocation_id') || randomUUID()
  const exists = Boolean(value(formData, 'advocation_id'))
  ...
  await ensureEntitySlug(supabase, slug, exists ? id : null)

  const entityPayload = { name, slug, summary: nullable(formData, 'summary'), status: status(formData) }
  const subtypePayload = { entity_id: id, advocation_type: nullable(formData, 'advocation_type'), description: nullable(formData, 'description') }

  if (exists) {
    assertMutation(await supabase.from('entities').update(entityPayload).eq('id', id).eq('entity_type', 'advocation'), 'No se pudo actualizar la Advocación')
    assertMutation(await supabase.from('advocations').upsert(subtypePayload, { onConflict: 'entity_id' }), 'No se pudo actualizar la ficha de Advocación')
  } else {
    assertMutation(await supabase.from('entities').insert({ id, entity_type: 'advocation', ...entityPayload }), 'No se pudo crear la Advocación')
    const subtypeResult = await supabase.from('advocations').insert(subtypePayload)
    if (subtypeResult.error) {
      await supabase.from('entities').delete().eq('id', id)
      throw new Error(`No se pudo crear la ficha de Advocación: ${subtypeResult.error.message}`)
    }
  }

  await audit(supabase, user, { action_type: exists ? 'update' : 'create', object_type: 'advocation', object_id: id, entity_id: id, summary: `${exists ? 'Advocación actualizada' : 'Advocación creada'}: ${name}`, changed_fields: { entity: entityPayload, advocation: subtypePayload } })
  refreshMasterData()
...
export async function saveMunicipalityAction(formData) {
  const user = await requirePanelEditor()
  const supabase = await createClient()
  const id = optionalUuid(formData, 'municipality_id')
  const name = required(formData, 'name', 'El municipio')
  const payload = {
    name,
    slug: slugify(required(formData, 'slug', 'El slug')),
  ...
  const result = id
    ? await supabase.from('municipalities').update(payload).eq('id', id).select('id').single()
    : await supabase.from('municipalities').insert(payload).select('id').single()
  const saved = assertMutation(result, 'No se pudo guardar el municipio')
  await audit(supabase, user, { action_type: id ? 'update' : 'create', object_type: 'municipality', object_id: saved.id, summary: `${id ? 'Municipio actualizado' : 'Municipio creado'}: ${name}`, changed_fields: payload })
  refreshMasterData()
  redirectSaved('municipios')
}

export async function savePlaceAction(formData) {
  const user = await requirePanelEditor()
  const supabase = await createClient()
  const id = optionalUuid(formData, 'place_id')
  const name = required(formData, 'name', 'El lugar')
  const payload = {
    ...
    opening_hours_verified_at: optionalDate(formData, 'opening_hours_verified_at'),
  }
  const result = id
    ? await supabase.from('places').update(payload).eq('id', id).select('id').single()
    : await supabase.from('places').insert(payload).select('id').single()
  const saved = assertMutation(result, 'No se pudo guardar el lugar')
  await audit(supabase, user, { action_type: id ? 'update' : 'create', object_type: 'place', object_id: saved.id, summary: `${id ? 'Lugar actualizado' : 'Lugar creado'}: ${name}`, changed_fields: payload })
  refreshMasterData()
  redirectSaved('lugares')
}
