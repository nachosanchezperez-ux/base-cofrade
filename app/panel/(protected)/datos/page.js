...
export default async function PanelMasterDataPage({ searchParams }) {
  ...
  return <div className={styles.pageWrap}>
    <header className={styles.pageHeader}>
      <div><span className={styles.eyebrow}>Datos maestros</span><h1>Datos</h1><p>Advocaciones, municipios y lugares reutilizados por el grafo de Hilo Cofrade.</p></div>
    </header>
    ...
    <nav className={styles.sectionTabs} aria-label="Secciones de datos"><a href="#advocaciones">Advocaciones</a><a href="#municipios">Municipios</a><a href="#lugares">Lugares</a></nav>
    ...
    <section className={styles.editorSection} id="advocaciones">
      ...
      <div className={styles.editorStack}>
        {data.advocations.map((item) => <article className={styles.editorItem} key={item.id}>
          ...
          <form action={saveAdvocationAction} className={styles.editorForm}><input type="hidden" name="advocation_id" value={item.id} /><div className={styles.formGrid}>
            <label className={styles.fieldWide}><span>Nombre</span><input name="name" defaultValue={item.name} required /></label>
            <label><span>Tipo de advocación</span><input name="advocation_type" defaultValue={item.advocation_type || ''} placeholder="Mariana, cristológica…" /></label>
            <label><span>Slug</span><input name="slug" defaultValue={item.slug || ''} required /></label>
            <label><span>Estado editorial</span><select name="status" defaultValue={item.status}><option value="draft">Borrador</option><option value="review">En revisión</option><option value="published">Publicado</option><option value="archived">Archivado</option></select></label>
    ...
    <section className={styles.editorSection} id="municipios">
      <div className={styles.sectionHeading}><div><span className={styles.eyebrow}>Geografía</span><h2>Municipios</h2></div><p>Fuente única para Hermandades, Bandas, Lugares y salidas.</p></div>
    ...
    <section className={styles.editorSection} id="lugares">
      <div className={styles.sectionHeading}><div><span className={styles.eyebrow}>Sedes y enclaves</span><h2>Lugares</h2></div><p>Parroquias, iglesias, plazas, templos y espacios con localización reutilizable.</p></div>
