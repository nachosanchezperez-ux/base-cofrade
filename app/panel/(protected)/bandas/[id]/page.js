...
function BandHeritageAssetForm({ item, data, canEdit }) {
  ...
  return <article className={`${styles.editorItem} ${styles.assetEditorItem}`}>
    <div className={styles.itemHeading}><div><span className={styles.eyebrow}>{isNew ? 'Nueva pieza' : item.asset_type || 'Patrimonio'}</span><h3>{isNew ? 'Añadir patrimonio de la banda' : entity.name}</h3></div>{!isNew ? <span className={`${styles.statusBadge} ${styles[entity.status]}`}>{STATUS_LABELS[entity.status]}</span> : null}</div>
    <form action={saveBandHeritageAssetAction} className={styles.editorForm}>
      <input type="hidden" name="band_id" value={data.entity.id} /><input type="hidden" name="asset_entity_id" value={entity.id || ''} />
      <div className={styles.formGrid}>
        ...
        <label><span>Tipo</span><input name="asset_type" defaultValue={item?.asset_type || ''} placeholder="Banderín, guion…" required /></label>
        ...
        <label className={styles.checkField}><input name="is_featured" type="checkbox" defaultChecked={item?.is_featured ?? true} /><span>Destacar públicamente</span></label>
    ...
    {!isNew && canEdit && entity.status !== 'archived' ? <form action={archiveBandHeritageAssetAction} className={styles.archiveForm}><input type="hidden" name="band_id" value={data.entity.id} /><input type="hidden" name="asset_entity_id" value={entity.id} /><button type="submit">Archivar esta pieza</button></form> : null}
  </article>
}
...
export default async function BandEditorPage({ params, searchParams }) {
  ...
  return <div className={styles.pageWrap}>
    ...
    <nav className={styles.sectionTabs} aria-label="Secciones de la ficha"><a href="#general">General</a><a href="#patrimonio">Patrimonio</a><a href="#acompanamientos">Acompañamientos</a><a href="#extraordinarias">Extraordinarias</a><a href="#estrenos">Estrenos</a><a href="#direccion">Dirección</a></nav>
    ...
    <section className={styles.editorSection} id="patrimonio"><div className={styles.sectionHeading}><div><span className={styles.eyebrow}>Memoria material</span><h2>Patrimonio de la banda</h2></div><p>Piezas opcionales como el banderín, con su imagen, realización, talleres y restauraciones.</p></div><div className={styles.editorStack}>{data.assets.map((item) => <BandHeritageAssetForm key={item.entity.id} item={item} data={data} canEdit={canEdit} />)}{canEdit ? <BandHeritageAssetForm data={data} canEdit /> : null}</div></section>
