...
function AccompanimentForm({ item, data, canEdit, defaultCurrent = true, sectionId = 'acompanamientos' }) {
  const isNew = !item?.id
  const isCurrent = item?.is_current ?? defaultCurrent
  return <article className={styles.editorItem}>
    <div className={styles.itemHeading}>
      <div>
        <span className={styles.eyebrow}>{isNew ? 'Nuevo acompañamiento' : item.outing_type}</span>
        <h3>{isNew ? `Añadir acompañamiento ${isCurrent ? 'actual' : 'histórico'}` : item.brotherhood?.name}</h3>
      </div>
    ...
    <form action={saveBandAccompanimentAction} className={styles.editorForm}>
      ...
      <input type="hidden" name="step_entity_id" value={item?.step_entity_id || ''} />
      ...
      <SaveBar label={isNew ? 'Crear acompañamiento' : 'Guardar acompañamiento'} canEdit={canEdit} />
    </form>
    {!isNew && item.status !== 'archived' && canEdit ? <form action={archiveBandAccompanimentAction} className={styles.archiveForm}>
      <input type="hidden" name="band_id" value={data.entity.id} />
      <input type="hidden" name="period_id" value={item.id} />
      <input type="hidden" name="return_section" value={sectionId} />
...
export default async function BandEditorPage({ params, searchParams }) {
  ...
  return <div className={styles.pageWrap}>
    ...
    <nav className={styles.sectionTabs} aria-label="Secciones de la ficha"><a href="#general">General</a><a href="#banderin">Banderín</a><a href="#enlaces">Enlaces</a><a href="#acompanamientos">Acompañamientos</a><a href="#acompanamientos-historicos">Histórico</a><a href="#extraordinarias">Extraordinarias</a><a href="#estrenos">Estrenos</a><a href="#direccion">Dirección</a></nav>
    ...
    <section className={styles.editorSection} id="acompanamientos"><div className={styles.sectionHeading}><div><span className={styles.eyebrow}>Semana Santa</span><h2>Acompañamientos actuales</h2></div><p>Relaciones que continúan vigentes en la actualidad.</p></div><div className={styles.editorStack}>{currentPeriods.map((item) => <AccompanimentForm key={item.id} item={item} data={data} canEdit={canEdit} />)}{canEdit ? <AccompanimentForm data={data} canEdit /> : null}</div></section>
    <section className={styles.editorSection} id="acompanamientos-historicos"><div className={styles.sectionHeading}><div><span className={styles.eyebrow}>Memoria musical</span><h2>Acompañamientos históricos</h2></div><p>Periodos ya finalizados que forman parte de la trayectoria de la banda.</p></div><div className={styles.editorStack}>{historicalPeriods.map((item) => <AccompanimentForm key={item.id} item={item} data={data} canEdit={canEdit} defaultCurrent={false} sectionId="acompanamientos-historicos" />)}{canEdit ? <AccompanimentForm data={data} canEdit defaultCurrent={false} sectionId="acompanamientos-historicos" /> : null}</div></section>
