...
function SocialLinkForm({ item, bandId, canEdit, excludedPlatforms = [] }) {
  const isNew = !item?.id
  ...
  return (
    <form action={saveBandSocialLinkAction} className={`${styles.editorItem} ${styles.editorForm}`}>
      ...
      <div className={styles.formGrid}>
        <label><span>Plataforma</span><select name="platform" defaultValue={item?.platform || ''} disabled={!isNew} required><option value="">Selecciona una plataforma</option>{platforms.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>{!isNew ? <input type="hidden" name="platform" value={item.platform} /> : null}</label>
      ...
      <SaveBar label={isNew ? 'Añadir enlace' : 'Guardar enlace'} canEdit={canEdit} note={isNew ? 'Las plataformas ya vinculadas no vuelven a ofrecerse.' : null} />
    </form>
  )
}
  ...
  return (
    <div className={styles.pageWrap}>
      ...
      <section className={styles.editorSection}>
        ...
        <div className={styles.editorStack}>{data.socialLinks.map((item) => <SocialLinkForm key={item.id} item={item} bandId={data.entity.id} canEdit={canEdit} />)}{canEdit && usedPlatforms.length < SOCIAL_PLATFORMS.length ? <SocialLinkForm bandId={data.entity.id} canEdit excludedPlatforms={usedPlatforms} /> : null}</div>
      </section>
    </div>
  )
}
