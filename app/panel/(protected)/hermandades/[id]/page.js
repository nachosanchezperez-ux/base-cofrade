...
function HeritageForm({ item, data, canEdit }) {
  const isNew = !item?.id
  return (
    <article className={styles.editorItem}>
  ...
  return (
    <div className={styles.pageWrap}>
      ...
      <section className={styles.editorSection} id="patrimonio">
        <div className={styles.sectionHeading}><div><span className={styles.eyebrow}>Actualidad patrimonial</span><h2>Estrenos y restauraciones</h2></div><p>Novedades documentadas por fecha, disciplina y elemento afectado.</p></div>
        <div className={styles.editorStack}>{data.heritage.map((item) => <HeritageForm key={item.id} item={item} data={data} canEdit={canEdit} />)}{canEdit ? <HeritageForm data={data} canEdit /> : null}</div>
      </section>
    </div>
  )
}
