        ...
        <section className={styles.editorSection}>
          ...
          <form action={createImageInterventionAction} className={`${styles.panelCard} ${styles.editorForm}`}>
            ...
            <div className={styles.formGrid}>
              ...
              <EntityPicker className={styles.fieldWide} name="agent_entity_id" items={data.agentOptions} label="Responsable identificado (opcional)" placeholder="Buscar Agente…" emptyLabel="Responsable no identificado" />
        ...
        <div style={{ display: 'grid', gap: 16 }}>
          {data.interventions.length ? data.interventions.map((item) => (
            <article className={styles.panelCard} key={item.id}>
              ...
              <h3>{item.agent?.name || 'Responsable no identificado'}</h3>
              <p className={styles.emptyText}>{[item.date_from_text, item.date_to_text].filter(Boolean).join(' → ') || 'Fecha pendiente de documentar'}{item.description ? ` · ${item.description}` : ''}</p>
