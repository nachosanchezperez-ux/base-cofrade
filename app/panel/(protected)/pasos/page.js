...
import { getPanelSteps } from '@/lib/panel/steps'
import styles from '@/app/panel/panel.module.css'

const STATUS_LABELS = { published: 'Publicado', review: 'En revisión', draft: 'Borrador', archived: 'Archivado' }

export const metadata = { title: 'Pasos · Panel' }

export default async function PanelStepsPage({ searchParams }) {
    ...
    <div className={styles.pageWrap}>
      <header className={styles.pageHeader}>
        <div>
          ...
          <h1>Pasos</h1>
          <p>Identidad básica de los pasos procesionales registrados en Hilo Cofrade.</p>
        </div>
        {canEdit ? <Link className={styles.primaryButton} href="/panel/pasos/nuevo">Nuevo paso</Link> : null}
      </header>

      <form className={styles.filters}>
        <label>
          <span className={styles.srOnly}>Buscar paso</span>
          <input type="search" name="q" defaultValue={q} placeholder="Buscar paso…" />
        </label>
        <label>
      ...
      <section className={styles.panelCard}>
        <div className={styles.listHeading}>
          <strong>{steps.length} pasos</strong>
          ...
          <div className={styles.brotherhoodList}>
            {steps.map((item) => (
              <article key={item.id}>
                ...
                <Link className={styles.rowLink} href={`/panel/pasos/${item.id}`}>Editar <span>→</span></Link>
              </article>
            ))}
          </div>
        ) : <p className={styles.emptyText}>No hay pasos que coincidan con este filtro.</p>}
      </section>
    </div>
  )
}
