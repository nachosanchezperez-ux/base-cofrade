...
const PANEL_ROUTES = { brotherhood: 'hermandades', image: 'imagenes', step: 'pasos' }
...
function percentage(formData, name, fallback = 50) {
  ...
  if (!Number.isFinite(parsed) || parsed < 0 || parsed > 100) throw new Error(`${name} debe estar entre 0 y 100.`)
  return parsed
}
...
export async function saveEntityCoverAction(formData) {
  ...
  if (!RIGHTS_STATUSES.has(rightsStatus)) throw new Error('Selecciona un estado de derechos publicable.')
  ...
  if (hasNewUpload) {
    const storagePattern = new RegExp(`^${entityId}\/[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}\.(?:jpg|png|webp|avif)$`, 'i')
  ...
  if (hasNewUpload) {
    ...
    if (currentCover) {
      const demoteResult = await supabase.from('entity_media').update({ is_cover: false }).eq('id', currentCover.id).eq('entity_id', entityId)
    ...
    const relationResult = await supabase.from('entity_media').insert({
      entity_id: entityId,
    ...
    assertMutation(await supabase.from('media_assets').update(assetPayload).eq('id', currentCover.media_asset_id), 'No se pudieron guardar los datos de la fotografía')
    assertMutation(await supabase.from('entity_media').update(relationPayload).eq('id', currentCover.id).eq('entity_id', entityId), 'No se pudo guardar el encuadre')
  }
