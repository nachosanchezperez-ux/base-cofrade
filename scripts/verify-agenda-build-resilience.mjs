...
// Ejecutar aparte de npm test: construye únicamente /agenda-cofrade.
...
const backend = createServer((request, response) => {
  ...
  let rows = []
  if (url.pathname === '/rest/v1/outings' && url.searchParams.get('outing_type')?.includes('rosario')) {
    rows = [{ id: 'qa-rosary', slug: 'qa-rosary', title: 'Rosario de verificación local', outing_type: 'Rosario', outing_date: '2099-10-10', departure_time: '18:00', return_time: '20:00', municipality_id: 'qa-town', status: 'published', event_status: 'announced', organizer_name: 'Entidad de prueba' }]
  } else if (url.pathname === '/rest/v1/municipalities') {
    rows = [{ id: 'qa-town', name: 'Sevilla', province: 'Sevilla' }]
  }
...
let server
try {
  const build = spawn(process.execPath, [next, 'build', '--debug-build-paths=app/agenda-cofrade/page.js'], { cwd, env, stdio: ['ignore', 'pipe', 'pipe'] })
  let output = ''
  ...
  assert.match(output, /ƒ \/agenda-cofrade/)
  console.log('PASS: build de Agenda con backend fallido y cero consultas')
  ...
  const page = `http://127.0.0.1:${port}/agenda-cofrade`
  ...
  assert.match(html, /Rosario de verificación local/)
  assert.match(html, /rel="canonical" href="https:\/\/hilocofrade.es\/agenda-cofrade"/)
  const afterRecovery = requestCount
  failing = true
  const cached = await fetch(page, { signal: AbortSignal.timeout(15000) })
  assert.equal(cached.status, 200)
  assert.match(await cached.text(), /Rosario de verificación local/)
